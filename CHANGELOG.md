...
- Profile deletion confirmation now expands inside its menu profile card instead of presenting a system confirmation dialog that can dismiss the MenuBarExtra before the user can respond. The profile list reserves enough additional height to keep both confirmation actions visible; Cancel and `Esc` clear only the pending confirmation.
