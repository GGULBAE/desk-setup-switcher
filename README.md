...
> **Pre-release 0.1.0 implementation candidate:** Tray Surface Architecture v2 resets its AppKit viewport on every open and attaches the top-scroll request only after the popover is shown, without inserting a phantom first-row spacer. Settings and workflow destinations temporarily participate in the ordinary macOS app/window cycle while visible and survive app switching and red-close/reopen cycles. The profile editor is intentionally reduced to Display, Audio, and Network and no longer exposes the ambiguous current-settings draft refresh action; unsupported color-mode and service-specific IPv4 writes remain visibly disabled instead of claiming unsafe support.
...
| --- | --- |
| Current tray/settings refinement | `make verify` passed on 2026-07-16 with localization/policy lint, 339 default cases (132 XCTest + 207 Swift Testing, six opt-in skips), Swift Debug/Release, universal Xcode Debug/Release, Analyze, DMG/checksum, mounted metadata/resources/architectures, and ad-hoc signature classification. Focused coverage includes post-attach top scrolling, 20 tray reopen cycles, two-window activation ownership, 10-cycle Settings/Profile Edit red-close recovery, input-volume rollback, and network identity ambiguity |
| Current refinement UI evidence | [Tray and Settings refinement audit](docs/TRAY-SETTINGS-REFINEMENT-AUDIT-2026-07-15.md) records 12 refreshed tray PNGs plus five profile-editor PNGs for Display, Korean Audio, dark-mode Network, 680×480 minimum width, and accessibility large text. They show the first-card spacing correction and removal of the draft-refresh action in detached content only; installed status-item/popover/window ordering, native localization context, keyboard traversal, VoiceOver, and hardware behavior remain manual |
| Current refinement local package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID status. SHA-256: `567917f169e90799db177d0a5f22a8b13115cb30ed63f7e766fc4bb992ab35e3`. It was not installed or launched |
...
The next task is a narrow **installed interaction audit**: open the actual tray 20 times, switch applications and Spaces with Settings/Profile Edit visible, verify red-close/reopen and frontmost ordering, and check keyboard/VoiceOver plus bilingual native rendering. Hardware setting mutation remains a separate opt-in procedure.
