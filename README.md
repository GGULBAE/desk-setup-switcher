...
- Every tray open creates a new top-anchored scroll identity and retains a zero-origin hosting viewport. A public AppKit container filters the native popover safe area before SwiftUI's first attached proposal, keeping the 16-point root inset horizontally symmetric without erasing native top/bottom chrome exclusion. Cached readiness stays visible during refresh, duplicate status presentations are ignored, and status-item title/width changes are deferred until the popover closes so its menu-bar anchor cannot move underneath it.
...
The authoritative scope is [docs/PRODUCT.md](docs/PRODUCT.md). Architecture and safety boundaries are in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) and [docs/TECHNICAL-SPEC.md](docs/TECHNICAL-SPEC.md). The current UI audit is [docs/UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md](docs/UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md), and the preceding load/save/apply audit is [docs/SETTINGS-LIFECYCLE-REFACTOR-2026-07-16.md](docs/SETTINGS-LIFECYCLE-REFACTOR-2026-07-16.md).
...
| --- | --- |
| Current native UI structural audit | [Native UI structural reliability audit](docs/UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md) records the persistent installed symptom, first-proposal AppKit safe-area fix, destination-window cancellation/liveness audit, minimum-size responsive workflow, keyboard/VoiceOver focus intent, synthetic evidence, and remaining installed/manual boundary |
| Current focused UI regression gate | The final non-live UI regression run passed 52 tests across five popover, geometry, destination-window, responsive-workflow, and attached-evidence suites. The native `T3/L11/B5/R2` fixture reaches SwiftUI as `T3/L0/B5/R0`, with content at `x16…352/y21…297`; the unfiltered raster control moves while the filtered asymmetric raster matches baseline |
...
| Current full local gate | `make verify` passed on 2026-07-17 with localization/policy lint and 412 default cases: 144 XCTest cases with five opt-in skips plus 268 Swift Testing cases across 35 suites with two default-disabled opt-in cases. Swift Debug/Release, universal Xcode Debug/Release, Analyze, DMG/checksum creation, mounted metadata/resources/architectures, and ad-hoc/no-Developer-ID classification all passed. No live flag was set |
...
| Current structural-fix local package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID status. SHA-256: `84bcdeac1f44de6381f93c8b9650132e26c4e9126c3d7137276db9c43274bb86`. This package was not installed or launched |
...
The next task is a separately authorized, non-mutating installed native UI confirmation: inspect the first and 20 repeated tray opens, red-close→immediate-reopen Settings/workflow behavior, keyboard-only traversal, and VoiceOver initial focus in English/Korean at the minimum window sizes. Do not confirm Apply Profile or change TCC, login, display, ColorSync, audio, network, mouse, or keyboard settings. Profile-store path/no-follow hardening resumes after this native UI gate.
