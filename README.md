...
> **Pre-release 0.1.0 implementation candidate:** the native app, safe profile core, concrete macOS adapters, protected profile-editing UI, and no-Developer-ID packaging pipeline are implemented. The current tray/editor follow-up completes inline profile deletion, measured-content tray sizing, focused capture/save feedback, and a genuinely resizable owned Settings window. Sixteen English/Korean PNGs and sixteen read-only AX logs pass static layout, localization, accessibility-structure, and privacy review. User-authorized installed-app interactions additionally pass delete Esc/Cancel/Confirm, 980→680→980 draft/selection/disclosure/focus preservation, Settings reactivation, and the Capture privacy explanation's stable System-window handoff; the complete local store was restored afterward. The measured-height correction passed the full non-live gate and its verified package replaced the local app without changing profile storage; a fresh installed visual check remains pending. No release is published, and live setting mutations, physical Intel, a live Keychain write, the downloaded/quarantined Gatekeeper path, full VoiceOver, and completed TCC grant/denial walkthroughs remain unverified.
...
- The popover header provides compact Capture plus icon-only, accessibility-labelled Settings and Quit actions. Each profile keeps one state-aware primary action, direct Edit, and a named destructive Delete control. Delete confirmation expands inside the profile card, and the profile list follows its measured card content up to a scrolling cap instead of estimating height from profile count; Cancel and `Esc` leave the profile untouched. `Apply…` handles a complete normal plan and `Apply Available…` handles a partial plan with executable operations. Both open an explicit preview; ellipsis, a separate availability-review button, and manual refresh are absent.
...
| --- | --- |
| Current tray/responsive-settings follow-up | The measured-content tray correction passed `make verify` on 2026-07-15: lint/localization policy; 305 default cases with six opt-in skips and zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted metadata/resources/architectures; and ad-hoc signature classification. Separately, `git diff --check` passed. The ordinary gate used no live flag |
| UI evidence | [Manual UI audit](docs/MANUAL-UI-AUDIT-2026-07-14.md) records 16 English/Korean PNGs plus 16 nonempty, read-only AX logs. Current deletion/capture feedback and standard/minimum editor states pass visual/AX/privacy review. [Installed deletion/resize evidence](docs/evidence/manual-ui-audit-2026-07-15/18-installed-delete-resize.txt) records Esc/Cancel/Confirm and 980→680→980 state preservation with full store rollback; the earlier [permission-handoff log](docs/evidence/manual-ui-audit-2026-07-14/13-installed-permission-handoff.txt) remains valid. The full denied/granted TCC matrix remains pending |
| Current local universal package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID signature status. SHA-256: `8bf4d547fae0df3cbe999db84e7be169b33d495b3993cf7c37f46ba37d6ea71d` |
| Current installed app | The existing `/Applications/Desk Setup Switcher.app` bundle was stopped, removed, and replaced from the verified DMG. Its universal executable matches the mounted current-DMG executable byte-for-byte and launched successfully. Existing profile storage was preserved; the measured-height build has not yet received a fresh installed visual walkthrough |
...
| Remaining manual checks | The current-tree static screenshot/AX walkthrough, actual MenuBarExtra inline deletion, Settings open/reopen, and same-window resize state/focus preservation are complete. Full VoiceOver/keyboard-order/contrast/real-text-size audit, approval-required and failure/retry login-item states, actual login-at-boot after a reboot, import/export, the full TCC permission denial/grant matrix, Gatekeeper/quarantine, physical Intel, and mutation/rollback procedures are pending |
...
The verified current DMG completely replaced the `/Applications` app bundle and the installed binary matched the mounted source. The user-authorized installed check exercised only tray deletion and Settings presentation/state; it applied no profile and changed no system setting. The complete local store was backed up and restored byte-for-byte. The 2026-07-11 baseline artifact retains the recorded login-item checks, including default-on registration and UI opt-out/re-enable. Nothing is published; quarantine/Gatekeeper, approval-required and retry states, and actual login-at-boot after a reboot remain untested. Developer ID signing and notarization are optional external release-operator work, not current project claims. See [docs/DISTRIBUTION.md](docs/DISTRIBUTION.md).
