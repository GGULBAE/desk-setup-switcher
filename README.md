...
> **Pre-release 0.1.0 implementation candidate:** the native app, safe profile core, concrete macOS adapters, protected profile-editing UI, and no-Developer-ID packaging pipeline are implemented. The current tray/editor follow-up adds per-profile deletion, value-free itemized partial-capture explanations, a wider card-based editor, and a reduced Profiles/System/About information architecture with diagnostics available on demand. Its integrated non-live `make verify` passed on 2026-07-14 with 298 default cases (129 XCTest plus 169 Swift Testing cases, including six opt-in skips), zero failures, universal builds, Analyze, and package verification. The DEBUG-only synthetic host, hard system-access suppression, deterministic English/Korean runtime localization, and de-duplicated profile-row accessibility remain in place. Twelve current-source English/Korean PNGs and twelve read-only AX logs passed static layout, localization, accessibility-structure, and privacy review. The local commit, installed-app replacement, and clean worktree are reported in the final handoff; no push or CI run is part of this local follow-up. The 297-case layout-correction package and UI-hardening commit `5f0cabc` remain historical evidence. No release is published, and live setting mutations, physical Intel, a live Keychain write, the downloaded/quarantined Gatekeeper path, full VoiceOver, and TCC workflows remain unverified.
...
- Capture produces a typed complete/partial/failure summary. The menu can disclose sanitized item names and separate snapshot-only, unreadable, permission-required, and unsupported reasons without retaining captured values or runtime device identifiers. Unsupported or permission-limited values remain non-secret snapshot evidence instead of making every persisted snapshot appear complete.
...
| --- | --- |
| Current tray/editor follow-up | `make verify` passed on 2026-07-14: lint/localization policy; 298 default cases (129 XCTest plus 169 Swift Testing cases, six opt-in skips), zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted metadata/resources/architectures; and ad-hoc signature classification. Separately, `git diff --check` passed. No live flag was used |
| Current synthetic UI evidence | [Manual UI audit](docs/MANUAL-UI-AUDIT-2026-07-14.md) records 12 current-source English/Korean PNGs plus 12 nonempty, read-only AX logs. Static overview, standard editor, 680×480 minimum, simulated large-text, validation, System, and advanced-diagnostics states passed visual/AX/privacy review. The overview covers profile deletion and partial-capture explanations; the editor covers the wider card layout. Same-window resize state/focus, the actual MenuBarExtra Settings click/reopen path, keyboard-only traversal, and VoiceOver behavior remain pending |
| Current local universal package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID signature status. SHA-256: `f3d24ae95709d0db9de13ba6032eb63a1d19a5be89af15aa68244da9019afbde` |
