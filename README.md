...
The current productization polish keeps every readiness label and **Review Changes…** action readable in the fixed 368-point tray, moves destructive profile deletion into the secondary menu, and gives recoverable destination failures a direct **Try Again** action. Manual IPv4 values retain visible field labels, profile validation has a Korean minimum-window/large-text fixture, and regular-size editor rows use quieter, denser framing while accessibility layouts continue to stack. Apply Preview consolidates stable warnings into one Safety summary without moving the final decision ahead of the reviewed changes; dirty-draft prompts show the unsaved and target profiles as separate rows. System permission recovery stays above the fold at the minimum size, and About/error surfaces use centered scroll-safe composition. These are deterministic synthetic/offscreen improvements only; installed accessibility, hardware mutation/rollback, and release-distribution evidence remain required.
