...
| --- | --- |
| Current installed-layout/apply follow-up | `make verify` passed on 2026-07-16 with localization/policy lint, 370 default cases (134 XCTest, including five skips, + 236 Swift Testing, including two disabled opt-in cases), Swift Debug/Release, universal Xcode Debug/Release, Analyze, DMG/checksum, mounted metadata/resources/architectures, and ad-hoc signature classification. New focused coverage injects an asymmetric native safe area, proves a stable reviewed plan reaches its adapter exactly once, and proves a changed execution preflight returns to review without applying |
...
| Current end-to-end local package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID status. SHA-256: `e2927513543903b794bea08628064b29c12604e18706bef169c20b3e89760bcd`. It was reinstalled to `/Applications`; bundle identity, version, architectures, and signature passed read-only checks. It was not launched after replacement |
...
| Current installed app | User-provided 2026-07-16 screenshots exposed an asymmetric empty-state layout and an ambiguous Apply-to-review handoff in the preceding installed build. The follow-up package above replaced `/Applications/Desk Setup Switcher.app`, but native launch/reopen, final empty-state appearance, and actual user-confirmed setting mutation remain manual |
