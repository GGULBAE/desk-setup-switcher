...
> **Pre-release 0.1.0 implementation candidate:** the native app, safe profile core, concrete macOS adapters, protected profile-editing UI, and no-Developer-ID packaging pipeline are implemented. The current tray/editor follow-up completes inline profile deletion, focused capture/save feedback, and a genuinely resizable owned Settings window. Sixteen English/Korean PNGs and sixteen read-only AX logs pass static layout, localization, accessibility-structure, and privacy review. User-authorized installed-app interactions additionally pass delete Esc/Cancel/Confirm, 980→680→980 draft/selection/disclosure/focus preservation, Settings reactivation, and the Capture privacy explanation's stable System-window handoff; the complete local store was restored afterward. The local commit and final installed-package checksum are reported below; no push or CI run is part of this local follow-up. No release is published, and live setting mutations, physical Intel, a live Keychain write, the downloaded/quarantined Gatekeeper path, full VoiceOver, and completed TCC grant/denial walkthroughs remain unverified.
...
| --- | --- |
| Current tray/responsive-settings follow-up | `make verify` passed on 2026-07-15: lint/localization policy; 305 default cases with six opt-in skips and zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted metadata/resources/architectures; and ad-hoc signature classification. Separately, `git diff --check` passed. The ordinary gate used no live flag |
...
| Current local universal package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID signature status. SHA-256: `8422b042b793bf0845ac56e2dcbd9808075a0467226d79ee65408440736648d8` |
| Current installed app | The existing `/Applications/Desk Setup Switcher.app` bundle was stopped, removed, and replaced from the verified DMG. Its universal executable matches the mounted current-DMG executable byte-for-byte. Scoped interaction changed no system setting, and the complete profile/diagnostic store was restored byte-for-byte |
