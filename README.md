...
- Workflow decision, preview, confirmation, and result actions measure their ideal widths and stack at the 520×360 minimum or under accessibility text. Long profile-specific meaning remains in accessibility labels, operation/result rows switch from grids to vertical layout when needed, and each transition selects a heading plus an enabled safe Cancel/Revert/Close keyboard target.
...
The authoritative scope is [docs/PRODUCT.md](docs/PRODUCT.md). Architecture and safety boundaries are in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) and [docs/TECHNICAL-SPEC.md](docs/TECHNICAL-SPEC.md). The current UI audit is [UI/UX simplification and installed-app audit](docs/UI-UX-SIMPLIFICATION-INSTALLED-AUDIT-2026-07-17.md); the [native UI structural reliability audit](docs/UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md) and [load/save/apply audit](docs/SETTINGS-LIFECYCLE-REFACTOR-2026-07-16.md) are preceding evidence.
...
| --- | --- |
| Current UI/UX installed audit | [UI/UX simplification and installed-app audit](docs/UI-UX-SIMPLIFICATION-INSTALLED-AUDIT-2026-07-17.md) records the final root cause—application code resetting AppKit's attached wrapper origin—the wrapper-ownership fix, persistent-window sizing and async-generation hardening, final package gate, installed geometry, safety invariants, and bounded P2 follow-up |
...
| Current end-to-end UI evidence | The [installed evidence index](docs/evidence/ui-ux-installed-2026-07-17/README.md) records the final installed screenshots, 20-open geometry, package hashes, and unchanged safety-state hashes. The preceding [tray and Settings end-to-end audit](docs/TRAY-SETTINGS-END-TO-END-AUDIT-2026-07-16.md) retains the broader synthetic matrix. Full keyboard traversal, VoiceOver, focused-control AX observation, and hardware behavior remain unproven |
| Current structural-fix package and install | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID status. DMG SHA-256 is `82b05e6a3b1b20978c85c4d82d0976237a731c76c07bcd7d1729a84e3e0b6f21`; the reinstalled `/Applications` executable SHA-256 is `40876bd671dd4286fa4684192097f6dbd702df899bccf8efb588b244c3d27305` |
...
| Remaining manual checks | Popover arrow/material/ghost-frame and outside-click/deactivation behavior, first responder and focused-control AX state, complete keyboard order beyond the recorded workflow Escape close, VoiceOver, real contrast/text-size/transparency settings, TCC denial/grant, Gatekeeper/quarantine, physical Intel, login approval/retry/reboot, import/export interaction, and mutation/rollback procedures remain pending |
...
The current follow-up DMG passed build, checksum, mount, resource, architecture, and signature verification, then was reinstalled to `/Applications` for the bounded non-mutating UI geometry check above. This does not prove full keyboard traversal, VoiceOver speech/rotor order, focused-control AX state, hardware mutation behavior, quarantine/Gatekeeper, login approval/retry, or actual login-at-boot. Nothing is published; Developer ID signing and notarization remain optional external release-operator work. See [docs/DISTRIBUTION.md](docs/DISTRIBUTION.md).

## Next task

The next task is the bounded P2 UI follow-up only: improve the empty-tray Capture call to action; settle Duplicate/Reorder/Import/Export hierarchy and dirty-Export semantics; remove Included-toggle ambiguity; verify `DisclosureGroup` with VoiceOver; reconcile Advanced Diagnostics' 700-point minimum with Settings' 680-point minimum; add minimum-size and accessibility fixtures for About, safety, and result states; verify `⌘,` tab forcing; and increase generic storage-error prominence. Full VoiceOver use and focused-control AX observation remain separate manual evidence. Do not confirm Apply Profile or invoke Capture, TCC, login, display, ColorSync, audio, network, mouse, or keyboard mutations while reviewing this scope.
