...
> **Pre-release 0.1.0 implementation candidate:** the native app, safe profile core, concrete macOS adapters, protected profile-editing UI, and no-Developer-ID packaging pipeline are implemented. The UI-audit follow-up adds a DEBUG-only synthetic host, hard system-access suppression, responsive Settings layout, deterministic English/Korean runtime localization, and de-duplicated profile-row accessibility. A subsequent editor-layout correction prevents `Form` switch alignment from collapsing Audio and option titles into narrow vertical text. Its integrated non-live `make verify` passed on 2026-07-14 with 297 default cases (129 XCTest plus 168 Swift Testing cases, including six opt-in skips), zero failures, universal builds, Analyze, and package verification. Twelve current-source English/Korean PNGs and twelve read-only AX logs passed static layout, localization, accessibility-structure, and privacy review. The behavior-focused local commit and clean worktree are reported in the final handoff rather than self-referenced here; no new push or CI run was performed. The 290-case apply-reliability package and UI-hardening commit `5f0cabc` remain historical evidence. No release is published, and live setting mutations, physical Intel, a live Keychain write, the downloaded/quarantined Gatekeeper path, full VoiceOver, and TCC workflows remain unverified.
...
| --- | --- |
| Current UI-audit follow-up | `make verify` passed on 2026-07-14: lint/localization policy; 297 default cases (129 XCTest plus 168 Swift Testing cases, six opt-in skips), zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted metadata/resources/architectures; and ad-hoc signature classification. Separately, `git diff --check` passed. No live flag was used |
| Current synthetic UI evidence | [Manual UI audit](docs/MANUAL-UI-AUDIT-2026-07-14.md) records 12 current-source English/Korean PNGs plus 12 nonempty, read-only AX logs. Static standard, 680×480 minimum, simulated large-text, validation, permission, and diagnostics states passed visual/AX/privacy review; the standard editor states explicitly cover corrected Audio/default-device title and Include-switch alignment. Same-window resize state/focus, the actual MenuBarExtra Settings click/reopen path, keyboard-only traversal, and VoiceOver behavior remain pending |
| Current local universal package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID signature status. SHA-256: `ae940ce1cffb969f309b8ffa8f6ffcd0637fd0845ee21bf24fa59129ea530ef7` |
