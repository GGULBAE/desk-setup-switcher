...
> **Pre-release 0.1.0 implementation candidate:** The tray now follows one deterministic first-attach contract, with fixed 0/1/2/overflow height tiers and no phantom first-row spacer. Settings exposes only complete Display, Audio, and Network vertical slices: every rendered control has typed capture, edit, validation, plan, apply, read-back, and rollback support; unavailable controls are absent rather than disabled promises.
...
| --- | --- |
| Current tray/settings end-to-end contract | `make verify` passed on 2026-07-16 with localization/policy lint, 366 default cases (134 XCTest + 232 Swift Testing, five opt-in skips), Swift Debug/Release, universal Xcode Debug/Release, Analyze, DMG/checksum, mounted metadata/resources/architectures, and ad-hoc signature classification. Focused coverage includes deterministic first attachment and 20 reopens, legacy activation normalization, the nine-kind visible-setting invariant, portable ColorSync targets, Core Audio device/volume ordering, authorized service IPv4, and protected rollback |
| Current end-to-end UI evidence | [Tray and Settings end-to-end audit](docs/TRAY-SETTINGS-END-TO-END-AUDIT-2026-07-16.md) records 12 tray PNG/AX pairs plus 10 current editor PNG/AX pairs covering Display/ColorSync, supported/unsupported Audio, Ethernet/Wi‑Fi DHCP/manual, English/Korean, light/dark, minimum width, and accessibility large text. They prove detached synthetic layout only; installed status-item/popover/window ordering, native localization context, keyboard traversal, VoiceOver, and hardware behavior remain manual |
| Current end-to-end local package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID status. SHA-256: `76bc6d9f1187ea30f68be16ee81ee4a334d877a4e26c2497f35a9ffc781678b3`. It was not installed or launched |
...
| Current installed app | A user-provided 2026-07-16 screenshot of the preceding installed v2 build exposed the first-card gap and draft-refresh action. The corrected package above was not installed or launched during this task, so visible reopen and application/Space-switching confirmation remain manual |
...
| Default test behavior | Five opt-in cases skip by default: four read-only hardware/context cases and one Keychain-write round trip. The dormant input read test is disabled rather than counted as a skip; no live setting mutation or Keychain write runs in the default suite |
...
The next task is an explicitly approved **installed and hardware verification audit**: first verify 20 real tray opens, native bilingual rendering, keyboard order, VoiceOver, and red-close/frontmost behavior; then, under the documented preflight/rollback procedure, verify ColorSync, Core Audio, and Ethernet/Wi‑Fi IPv4 mutations on representative hardware. Ordinary development and CI remain non-live.
