...
- Capture checks Location authorization before reading a Wi-Fi name. An undetermined state gets a privacy explanation followed by the macOS request; a denied/restricted state offers macOS System Settings or an explicit Wi-Fi-free capture. User-facing results retain applicable settings and actionable permission gaps only, while snapshot-only, unreadable, and unsupported evidence stays out of the error card. The always-unsupported network service-order read was removed entirely.
...
| --- | --- |
| Current capture-permission follow-up | `make verify` passed on 2026-07-14: lint/localization policy; 300 default cases (129 XCTest plus 171 Swift Testing cases, six opt-in skips), zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted metadata/resources/architectures; and ad-hoc signature classification. Separately, `git diff --check` passed. No live flag was used |
| Most recent synthetic UI evidence | [Manual UI audit](docs/MANUAL-UI-AUDIT-2026-07-14.md) records 12 English/Korean PNGs plus 12 nonempty, read-only AX logs. Static overview, standard editor, 680×480 minimum, simulated large-text, validation, System, and advanced-diagnostics states passed visual/AX/privacy review. Those captures predate the inline-delete and actionable capture-permission follow-ups; current user-driven destructive-action and permission click-throughs remain pending |
| Current local universal package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID signature status. SHA-256: `ed52b253159e6abc8fe35e606aed56cc269693a53b76986dae20a04ffb2bd4fc` |
| Current installed app | The existing `/Applications/Desk Setup Switcher.app` bundle was stopped, removed, and replaced from the verified DMG. The installed universal binary matched the mounted binary and launched successfully as PID-backed `DeskSetupSwitcher`; no setting mutation or UI automation was used. Existing profile storage was preserved |
...
| Current live read-only discovery | On 2026-07-14, all five opt-in display, audio, network, input, and readiness-context smoke tests passed on the current capture-permission tree on an Apple M5 Mac running macOS 26.5.2. The display path also passed while the online session display was asleep and Core Graphics correctly reported zero active displays. The live Keychain write remained skipped, and no setting mutation was run |
...
| Remaining manual checks | The current-tree static screenshot/AX walkthrough is complete. The actual MenuBarExtra Settings click/reopen path, same-window resize state/focus preservation, full VoiceOver/keyboard/focus/contrast/real-text-size audit, approval-required and failure/retry login-item states, actual login-at-boot after a reboot, import/export, TCC permission denial/grant, Gatekeeper/quarantine, physical Intel, and mutation/rollback procedures are pending |
