...
> **Pre-release 0.1.0 implementation candidate:** the native app, safe profile core, concrete macOS adapters, protected profile-editing UI, and no-Developer-ID packaging pipeline are implemented. The current tray/editor follow-up adds per-profile deletion, an actionable capture-permission flow, a wider card-based editor, and a reduced Profiles/System/About information architecture with diagnostics available on demand. Its integrated non-live verification status and current package checksum are recorded below. The DEBUG-only synthetic host, hard system-access suppression, deterministic English/Korean runtime localization, and de-duplicated profile-row accessibility remain in place. Twelve earlier English/Korean PNGs and twelve read-only AX logs passed static layout, localization, accessibility-structure, and privacy review; they predate the latest permission-flow simplification. A separate, user-authorized installed-app interaction verified Settings open/close/reopen and the Capture privacy explanation's stable handoff to the app System window, with the complete profile/diagnostic store restored afterward. The local commits, installed-app replacement, and clean worktree are reported in the final handoff; no push or CI run is part of this local follow-up. The 297-case layout-correction package and UI-hardening commit `5f0cabc` remain historical evidence. No release is published, and live setting mutations, physical Intel, a live Keychain write, the downloaded/quarantined Gatekeeper path, full VoiceOver, and completed TCC grant/denial walkthroughs remain unverified.
...
- The popover header provides compact Capture plus icon-only, accessibility-labelled Settings and Quit actions. Each profile keeps one state-aware primary action, direct Edit, and a named destructive Delete control. Delete confirmation expands inside the profile card, and the profile list grows to keep its confirmation actions visible without dismissing the MenuBarExtra; Cancel and `Esc` leave the profile untouched. `Apply…` handles a complete normal plan and `Apply Available…` handles a partial plan with executable operations. Both open an explicit preview; ellipsis, a separate availability-review button, and manual refresh are absent.
...
- Capture checks Location authorization before reading a Wi-Fi name. An undetermined state gets a privacy explanation followed by the macOS request; a denied/restricted state offers macOS System Settings or an explicit Wi-Fi-free capture. Every permission handoff first opens the app's persistent System tab before invoking TCC or macOS System Settings, so the MenuBarExtra focus change has a visible destination instead of looking like a silent dismissal. User-facing results retain applicable settings and actionable permission gaps only, while snapshot-only, unreadable, and unsupported evidence stays out of the error card. The always-unsupported network service-order read was removed entirely.
...
| --- | --- |
| Current capture-permission follow-up | `make verify` passed on 2026-07-14: lint/localization policy; 301 default cases (129 XCTest plus 172 Swift Testing cases, six opt-in skips), zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted metadata/resources/architectures; and ad-hoc signature classification. Separately, `git diff --check` passed. No live flag was used |
| UI evidence | [Manual UI audit](docs/MANUAL-UI-AUDIT-2026-07-14.md) records 12 English/Korean PNGs plus 12 nonempty, read-only AX logs. Static overview, standard editor, 680×480 minimum, simulated large-text, validation, System, and advanced-diagnostics states passed visual/AX/privacy review. Those captures predate the inline-delete and actionable capture-permission follow-ups. A separate [sanitized installed-interaction log](docs/evidence/manual-ui-audit-2026-07-14/13-installed-permission-handoff.txt) records the actual status item, Settings open/close/reopen, Capture purpose explanation, stable app System-window handoff, and complete test-store rollback. Inline deletion and the full denied/granted TCC matrix remain pending |
| Current local universal package | `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg` verified `x86_64 arm64`, English/Korean resources, `/Applications` link, mounted layout, and ad-hoc/no-Developer-ID signature status. SHA-256: `150d1e0bb620fba52c2ec2a6a78345b5b74f44ca10c9d5375178f6ab16ea370d` |
| Current installed app | The existing `/Applications/Desk Setup Switcher.app` bundle was stopped, removed, and replaced from the verified DMG. Its universal executable remains byte-for-byte identical to the newly rebuilt current-DMG executable and runs as PID-backed `DeskSetupSwitcher`; installation itself used no UI automation or setting mutation. A later user-authorized, scoped menu interaction changed no system setting and restored the complete profile/diagnostic store |
...
| Remaining manual checks | The current-tree static screenshot/AX walkthrough and actual MenuBarExtra Settings open/close/reopen are complete. Same-window resize state/focus preservation, inline deletion, full VoiceOver/keyboard/focus/contrast/real-text-size audit, approval-required and failure/retry login-item states, actual login-at-boot after a reboot, import/export, the full TCC permission denial/grant matrix, Gatekeeper/quarantine, physical Intel, and mutation/rollback procedures are pending |
...
The preceding UI-audit package's real Korean Settings rendering exposed the Audio-row compression corrected here. The verified current DMG completely replaced the running `/Applications` app bundle, the installed binary matched the mounted source, and the new app launched successfully. Existing profile storage was preserved because the current schema remains compatible. This smoke check did not interact with the menu UI or mutate a setting. The 2026-07-11 baseline artifact retains the recorded login-item checks, including default-on registration and UI opt-out/re-enable. Nothing is published; quarantine/Gatekeeper, approval-required and retry states, and actual login-at-boot after a reboot remain untested. Developer ID signing and notarization are optional external release-operator work, not current project claims. See [docs/DISTRIBUTION.md](docs/DISTRIBUTION.md).
