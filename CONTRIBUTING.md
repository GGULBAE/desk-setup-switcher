...
Before requesting review, configure GitHub to use a noreply email for both commit and annotated-tag metadata, then run `make verify` and `git diff --check`. Already-public legacy identity metadata is retained without history rewriting and is allowlisted only by exact immutable commit SHA; do not add names or addresses to that file. If a required manual or hardware test was not run, state that directly. Do not treat the ad-hoc integrity signature in the local DMG as Developer ID signing or notarization.
