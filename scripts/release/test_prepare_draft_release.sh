...
case "${1:-}" in
    rev-parse)
        case "${2:-}" in
            ...
            HEAD|"refs/tags/$MOCK_TAG^{commit}"|"refs/tags/$MOCK_TAG") printf '%s\n' "$MOCK_EXPECTED_COMMIT" ;;
            *) exit 91 ;;
        esac
        ;;
    remote)
    ...
    check-ref-format)
        ...
        ;;
    show-ref)
        [[ "$#" == 4 && "$2" == --verify && "$3" == --quiet && "$4" == "refs/tags/$MOCK_TAG" ]] || exit 94
        ;;
    ls-files)
...
case "${1:-} ${2:-}" in
    "api --method")
        [[ "$#" == 10 && "$3" == GET && "$4" == --paginate && "$5" == --slurp \
            && "$6" == -H && "$7" == "Accept: application/vnd.github+json" \
            && "$8" == -H && "$9" == "X-GitHub-Api-Version: 2022-11-28" \
            && "${10}" == "/repos/GGULBAE/desk-setup-switcher/releases?per_page=100" ]] || exit 81
    ...
    "release create")
        ...
        while [[ "$#" -gt 0 ]]; do
            case "$1" in
                --repo)
                    [[ "${2:-}" == GGULBAE/desk-setup-switcher ]] || exit 84
                    repo=true
                    shift 2
                    ;;
    ...
    "release upload")
        ...
        while [[ "$#" -gt 0 ]]; do
            case "$1" in
                --repo)
                    [[ "${2:-}" == GGULBAE/desk-setup-switcher ]] || exit 92
                    repo=true
                    shift 2
                    ;;
                --clobber) exit 93 ;;
    ...
    "release download")
        ...
        while [[ "$#" -gt 0 ]]; do
            case "$1" in
                --repo)
                    [[ "${2:-}" == GGULBAE/desk-setup-switcher ]] || exit 102
                    repo=true
                    shift 2
                    ;;
...
title="Desk Setup Switcher $tag signed candidate"
...
setup_case() {
    ...
    case "$scenario" in
        absent|interrupted-upload|api-failure|remote-tag-drift|remote-tag-object-drift|local-extra|local-symlink|local-nonregular) ;;
...
run_target_with_attempts() {
    ...
    local gh_token="${5-synthetic-test-token}"
    env -i \
        "PATH=$mock_bin:$PATH" \
        "HOME=${HOME:-/tmp}" \
        "TMPDIR=${TMPDIR:-/tmp}" \
        ...
        github_token=synthetic-inherited-lowercase-token \
        "RELEASE_TAG=$tag" \
