...
- Replace the opaque beta gate with descriptor-bound `external-beta/v2` reports and set, an actual-byte `candidate-inventory/v1`, and `predecessor-lineage/v3`. The contracts bind the retained protected `v0.0.9`/build-1 origin to the current `v0.1.0`/build-2 origin and require that exact upgrade in all three reports; the prior first-beta not-applicable branch is removed. Publication approval v2 consumes the verifier's descriptor-read digests for both release manifests/provenance records, inventory, lineage, set, ordered reports, and Sonoma report immediately before the sole PATCH. Remote-history completeness and real tester identity remain protected reviewer trust boundaries. This is deterministic local/mock policy evidence only—no inventory review, tester result, signing run, or publication has occurred.
...
- Complete separate exact-candidate evidence for browser download and real extracted-DMG quarantine, Gatekeeper without Open Anyway, first launch/login default-off, the mandatory protected `v0.0.9`/build-1 to `v0.1.0`/build-2 upgrade, schema 0→1 migration, backup recovery, import/export, diagnostics, uninstall, and optional app-owned data removal.
