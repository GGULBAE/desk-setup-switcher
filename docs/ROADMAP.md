...
The user-authorized [local reinstall](KEYBOARD-REINSTALL-2026-09-15.md) launched the current source from `/Applications` on Apple Silicon while preserving both profile files byte-for-byte. Because the installed Xcode license is not accepted, this was an `arm64` SwiftPM development bundle rather than the canonical verified universal DMG; it does not close the commit, integrated verification, distribution, or Keyboard hardware gates.
...
Publish only the bilingual informational holding page at <https://ggulbae.github.io/desk-setup-switcher/>. Keep `site/release-publication.json` in `holding` with a null release URL, so no download or install instructions can appear. The dedicated Next.js static export fixes the approved `github.io` origin and `/desk-setup-switcher` base path, writes `site/out`, and rejects non-holding release state. Source commit `fed2b05` passed both jobs in CI run `34700713290` and was rebuilt from a detached exact-commit worktree; generated commit `3da2b91` was published to the dedicated `gh-pages` branch. GitHub-managed Pages run `34702502028` succeeded; Pages reports the build complete with HTTPS enforced, deployed HTML/assets returned `200`, a clean browser loaded the page, and authenticated read-back confirmed the repository Homepage. No `master` push deploys automatically, and no project or Cloudflare secret is used. The owner-only Sites preview remains separate and `noindex`. GitHub's dynamic Pages workflow must still be explicitly incorporated into the exact-workflow remote release-controls policy before any app Release can pass that later gate.
...
Align the remote repository description, discovery topics, custom social preview, and exact Pages Homepage with the current seven-setting Display/Sound product and manual-only workflow. The deterministic `1280×640` preview uses the still-current synthetic Capture screen rather than the stale legacy editor. The approved holding page now passes deployed clean-link checks; Discussions stays disabled, and no public Release, directory listing, or community post is approved. The researched promotion sequence in [launch copy](LAUNCH-COPY.md) deliberately defers Threads and broad acquisition until a runnable public build exists; signing and notarization precede high-volume installation channels. `make verify-public-surface` and the full non-live `make verify` pass locally.
...
The integrated non-live `make verify` passes 194 XCTest cases, 333 Swift Testing cases across 39 suites, one isolated native popover regression, and 4,820 release-tooling assertions. Lint/localization policy, Swift Debug/Release, universal Xcode Debug/Release, Analyze, package, checksum, and mounted `arm64+x86_64` verification all pass. The local unsigned development DMG SHA-256 is `6a77ad1f830f30faa620a1e6f29ab1f9fb035c43ad43b9e6426ddcc58b355cff`. Its pre-sign build executable SHA-256 is `6b1b6feda3b886de30b5cbd2cc78117682c4d5e14d0d0932b53bb1bd0e5da260`, while the final ad-hoc-signed mounted executable SHA-256 is `6d67115a107fb35d098b6d8640cadb025a1e60d716235f5ae618b82614411d0a`. Behavior-focused commit `30b50b1` records the implementation and evidence.

A separately authorized reinstall moved the preceding app into the recoverable build-local backup `.build/reinstall-density.pPcc2H`, copied the verified mounted package to `/Applications`, detached the DMG, and launched only the installed path. The mounted and installed executables match at SHA-256 `6d67115a107fb35d098b6d8640cadb025a1e60d716235f5ae618b82614411d0a`; bundle identity, version `0.0.9`, `x86_64 arm64`, and the ad-hoc signature pass read-only checks. Profile primary and backup hashes remained unchanged. No app control, permission, login item, Keychain item, or system setting was changed.
...
- Harden profile storage and import against traversal, symlinks, non-regular files, wrong ownership, oversized inputs, source replacement, inherited extended ACLs, and tested file-identity/TOCTOU races. Descriptor-bound reads and identity-bound quarantine/permission repair preserve fail-closed behavior without turning unrelated permission denial into an application-wide failure. Managed directories/files and staging remove and absence-verify extended ACLs before enforcing `0700`/`0600`. Managed staging, commit, guarded rollback, cleanup, and sync remain on one verified parent-directory descriptor; missing targets use exclusive rename and existing targets use atomic swap. Export reuses the synchronized staging path in no-overwrite mode. Its selected-parent policy accepts current-user-owned non-shared-write or sticky directories and sticky root-owned compatibility directories such as macOS `/tmp`, while rejecting other owners, non-sticky shared-write directories, and any ALLOW ACL; deny-only ACLs remain compatible. Deterministic ACL, parent-path, regular/symlink/staging leaf replacement, export destination-race/special-entry/maximum-leaf/pre/post-commit cases, and both replacement rollback tests preserve displaced objects. Documented residuals are Darwin's lack of inode-conditional rename/unlink against a noncooperative same-UID process, possible staging residue after abrupt interruption or detected hostile replacement, and best-effort parent-directory durability—not a cooperative direct-to-final write or path-redirection fallback.
...
- Complete the source-level productization polish: preserve full readiness/action copy at 368 points, keep populated IPv4 fields visibly labelled, move profile deletion into the secondary menu, consolidate stable Apply warnings into one Safety summary, compare dirty-draft profile identities in labelled rows, expose safe destination retry, localize Korean beta/breadcrumb/validation copy consistently, and center sparse About/error content without weakening scroll or keyboard behavior. Deterministic unit and synthetic offscreen evidence do not close installed-accessibility, hardware, TCC, or distribution gates.
- Prepare the user-facing public surface locally: rewritten README, English/Korean user guides, profile-schema and adapter-contract references, launch copy, bilingual static site, and public media regenerated from exact committed application source `bc3ec58e2f1f13e3c47be3e879fed957564012cd`. The Capture/Edit/Review screenshots, silent bilingual-captioned demo, deterministic social preview, eight-file public manifest, 11-file source/AX manifest, and asset provenance are synchronized and locally verified; superseded unmanifested evidence was removed and the verifier now rejects unexpected source-evidence files.
...
- Pass integrated non-live `make verify` on 2026-07-22 with 5,298 deterministic checks/assertions against the current `v0.0.9`/build-1 source: 516 app checks (192 XCTest, 323 default Swift Testing cases across 39 suites, and one isolated native popover case in a 40th Swift Testing suite) plus 4,782 release-tooling assertions (374 base release-policy, 8 Mach-O compatibility-verifier, 659 remote-controls v1 policy/normalizer, 210 remote-controls v2 lifecycle-policy, 86 remote-controls v3 lifecycle-policy, 71 remote-controls v2 collector, 17 remote-controls v3 collector, 407 publication-approval policy, 1,386 external-beta/inventory/lineage/template policy, 132 collector-wrapper mock, 19 release-evidence history, 57 draft-reconciler mock, 261 artifact-restoration mock, 557 approved-publication mock, 306 legacy-workflow-containment mock, and 232 shell/workflow guard assertions). The regenerated `artifacts/Desk-Setup-Switcher-0.0.9-unsigned.dmg` has SHA-256 `2a743a6df6c14ad4b2ae3b1fc4ecc5bdeb7527089165d3ed7e6db32c1d5cf0c4`; it remains local development evidence, was not uploaded or published, and the DMG is not byte-for-byte reproducible. The exact `arm64+x86_64` set and per-slice macOS 14.0 `LC_BUILD_VERSION` fail closed and are byte-bound to the executable SHA-256 after signing and after final-DMG mounting. The 39-assertion public-release audit fixture suite and complete-history/current-asset production scan passed on 2026-07-22 after the current source and media commits. The release assertions do not prove a real inventory review, external tester result, credentialed signing/notarization, protected-remote, or publication run.
...
- Recheck that boundary with authenticated read-only GitHub queries on 2026-07-20, 2026-07-21, and 2026-07-22. At the 2026-07-22 13:48 KST observation, remote `master` was `1489b7fcb41ffa7e55a43ed65e6befc538838140`; the audited local release-control source `de4f805cfadf5fdfd1266d07286d27192794ca33` was its descendant by 39 commits. Remote workflows were only CI plus active historical Release workflow `311269012`; `master` was unprotected (`protected:false`, branch-protection 404), with zero rulesets, environments, repository Actions secret/variable names, `v*` tags, Releases, and Release-workflow runs. Immutable Releases, private reporting, and SHA pinning were disabled; Actions were unrestricted; Homepage/topics/Pages/`needs-triage` were absent, Discussions was disabled, and the description was stale. Public/MIT/Issues, the read-only workflow token, secret scanning, push protection, Dependabot security updates, and existing successful remote-master CI were enabled. This was blocker evidence, not a passing final-pre-tag gate, and it changed no remote state.
- Add a local fail-closed legacy-workflow containment helper. Its GET-only plan makes two full anchored observations and writes an external owner-only receipt; a separately approved apply revalidates that exact receipt, performs two fresh observations, permits at most one fixed bodyless disable PUT, requires 204/empty plus two stable post-observations, and writes a separate success receipt. Already-disabled is a zero-PUT success; any post-PUT uncertainty is exit 75 with no automatic retry and the helper contains no enable/tag/Release/push path. GitHub CLI 2.95-compatible paginated projection and strict JSON-line canonicalization pass 306 local assertions. On 2026-07-22, private uncommitted `legacy-workflow-containment-plan-de4f805-20260722.json` was created under a mode-0700 parent as a mode-0600 file, bound exact local HEAD `de4f805cfadf5fdfd1266d07286d27192794ca33`/four tool blobs plus active/zero-tag/zero-Release state, and passed a separate `validate-plan` invocation using the same checked-in validator at digest `bb53444e29871dd077d975128f0d41299d484d694429939a5e9f7daa11e0d4e6`. Apply accepts it only from a clean checkout of that bound HEAD; a later HEAD must create a new plan. No apply, PUT, push, tag, dispatch, or other remote mutation occurred; the receipt is planning evidence, not containment.
- Extend the fail-closed remote-controls verifier into a three-phase v3 lifecycle with strict synthetic fixtures and a read-only GitHub collector. It binds the exact legacy, signed-candidate, CI, and publication workflow identities/blobs plus the complete active-workflow inventory; both required CI checks, check runs, and jobs from one successful check suite and pinned workflow run; effective `master` and split release-tag rules; two protected environments; actor identities; separated credential-name scopes; Actions/security controls; and fresh phase-specific manual-record digests. `predecessor-pre-tag` requires zero version tags and Releases; `final-pre-tag` requires only the exact direct annotated predecessor tag and zero Releases; `pre-publication` requires both exact direct annotated-tag objects and one exact final draft prerelease. Every release-critical remote observation is collected twice and must normalize identically. The authoritative policy remains `configured:false` until its complete repository/workflow/operator/reviewer/publisher/approval identity is reviewed, and the unavailable API facts remain separate Settings/manual gates.
- Make signed-draft preparation recoverable without rebuilding: preserve the exact nine assets from both `v0.0.9`/build-1 and `v0.1.0`/build-2 in distinct immutable attempt-1 origin-run artifacts, then use a separate draft dispatch bound only to the final origin run, artifact ID, and archive digest. Verify each raw archive, exact files, signed candidate, and attestations; reject conflicting metadata/assets; and permit only additive upload of missing byte-identical final assets before full redownload verification. Neither origin build is ever rerun, the predecessor creates no Release, and no clobber, Release edit/delete/publication, or tag mutation is allowed.
- Add an approval-bound publication workflow and closed-schema policy that restore both exact attempt-1 candidates, prove the linear P→P1→F→F1 first-parent evidence histories and tag-bound digests, revalidate both approval-commit CI jobs plus both tags and the exact final draft/nine assets/attestations, actor and token ownership, remote-controls/manual evidence, canonical chronology, and immutable-release state immediately before one exact-ID PATCH. The predecessor is fully reverified from its tag's own code in a private detached worktree. After the transition it verifies the public prerelease and re-downloads all nine final assets. Any publication-side signal, cancellation, host loss, incomplete log, or ordinary failure after a PATCH attempt is explicitly incident-only. The policy and publisher assertions are local/mock evidence; no remote publication occurred.
- Replace the opaque beta gate with descriptor-bound `external-beta/v3` reports and `external-beta-set/v2`, an actual-byte `candidate-inventory/v1`, and `predecessor-lineage/v3`. The contracts bind the retained protected `v0.0.9`/build-1 origin to the current `v0.1.0`/build-2 origin and require that exact upgrade in all three reports; each report also binds Finder copies at exact `/Applications/Desk Setup Switcher.app`, DMG eject-before-launch, and installed identities from both release manifests; the prior first-beta not-applicable branch is removed. Publication approval v2 consumes the verifier's descriptor-read digests for both release manifests/provenance records, inventory, lineage, set, ordered reports, and Sonoma report immediately before the sole PATCH. Remote-history completeness and real tester identity remain protected reviewer trust boundaries. This is deterministic local/mock policy evidence only—no inventory review, tester result, signing run, or publication has occurred.
- Keep the policy-owned stdout-only rejected-placeholder generator to the four supported inventory, lineage, report, and set shapes. Generated JSON uses the exact production key structure but reserved non-evidence values, false gates, invalid IDs/times/digests, and no host or candidate input; `verify-set` rejects it explicitly, and the public-history audit rejects a tracked sentinel under release evidence. This closes the operator-scaffolding gap without claiming an inventory review, tester result, or digest lineage.
