...
- Pass integrated non-live `make verify` on 2026-07-18 with 1,993 deterministic checks/assertions: 501 app checks (178 XCTest, 322 default Swift Testing cases across 39 suites, and one isolated native popover case in a 40th Swift Testing suite) plus 1,492 release-tooling assertions (328 base release-policy, 659 remote-controls policy/normalizer, 79 collector-wrapper mock, 51 draft-reconciler mock, 234 artifact-restoration mock, and 141 shell/workflow guard assertions). The latest regenerated unsigned development-evidence DMG SHA-256 is `16ac9cbe010507c39b83afe1367bc383cab722bea00ea753dfcd89941e376d53`; the DMG is not byte-for-byte reproducible, and the release assertions do not prove a real credentialed signing/notarization run.
...
- Add an explicit fail-closed final-pre-tag verifier with strict synthetic fixtures and a read-only GitHub collector. It checks exact repository/release-workflow/CI-workflow identity, binds the required check suite to the pinned CI run/job, verifies effective `master` rules, split release-tag creation and no-bypass update/deletion rules, protected environment reviewer and credential-name scopes, Actions/security controls, authenticated draft visibility, and a zero-tag/zero-Release boundary. Every release-critical remote observation is collected twice and must normalize identically. The authoritative policy remains unconfigured until its complete repository/workflow/operator/reviewer/approval identity is reviewed, and the environment administrator-bypass switch remains a separate manual gate.
