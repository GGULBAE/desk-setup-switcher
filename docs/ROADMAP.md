...
Publish only the bilingual informational holding page at <https://ggulbae.github.io/desk-setup-switcher/>. Keep `site/release-publication.json` in `holding` with a null release URL, so no download or install instructions can appear. The dedicated Next.js static export fixes the approved `github.io` origin and `/desk-setup-switcher` base path, writes `site/out`, and rejects non-holding release state. A maintainer publishes reviewed output to the dedicated `gh-pages` branch; no `master` push deploys automatically, and no project or Cloudflare secret is used. The owner-only Sites preview remains separate and `noindex`. Local Pages build/tests are implementation evidence; the GitHub-managed branch deployment and clean-session checks remain separate remote evidence. GitHub's dynamic Pages workflow must be explicitly incorporated into the exact-workflow remote release-controls policy before any app Release can pass that later gate.
