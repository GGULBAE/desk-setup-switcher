...
Display, Audio, and Network use a stable three-step presentation without changing the stored document projection. Common display arrangement, sound output, and selected-service controls are visible first; resolution/refresh and audio input/mute are revealed inside Advanced settings, while exact DHCP/manual IPv4 remains directly editable under the Network connection card. Last-application metadata is retained in storage but not rendered by the editor. `ProfileEditorWorkspaceLayoutPolicy` uses only available width and accessibility text to replace the rail with a segmented selector; a section's intrinsic content width and disclosure expansion never participate. `ProfileEditorStepPolicy` maps validation field identifiers back to the correct step; `ProfileEditorNetworkSelectionPolicy` resolves service-indexed validation fields to the owning connection before focus transfer. Network connection selection is transient view state, while `ProfileEditorNetworkSelectionPolicy` changes inclusion only for the selected service and preserves every other included service. `VisibleSettingRegistry` declares nine setting kinds and requires capture/edit/validate/plan/apply/verify/rollback stages. Unsupported, missing, read-only, or ambiguous controls are absent as new choices. A persisted included Audio or service-IPv4 target that is no longer available is projected alongside related controls as a warning-only repair row whose remaining action is to turn Include off. Core Audio catalogs volume and output-mute controls per device, and the editor resolves each control against the draft's selected target UID. A single primary-display picker enforces exactly one included primary display. New display snapshots omit ColorSync options/catalogs, and normalization excludes legacy color leaves at capture, load/import, and Apply preparation boundaries while preserving their values for JSON compatibility. Legacy ColorSync validation/adapter primitives remain internal regression support, not a current profile feature. Manual IPv4 validates each selected service's address, contiguous mask, and optional router with service-indexed field IDs. `ProfileDraftValidator` returns stable identifiers and typed required/range/format issues before save; the app maps them to English/Korean messages, invalid-state accessibility metadata, first-error focus, and useful valid-state hints.
...
`LSUIElement` requests an accessory/menu-bar lifecycle. Production owns one `NSStatusItem`, one `.applicationDefined` `NSPopover`, and one `NSHostingController`; SwiftUI supplies contained tray content but does not own the surface lifecycle. `TrayGeometry` selects a 368-point-wide viewport with 260/260/316/480/560-point heights for 0/1/2/3/4+ profiles, clamped to the active screen. `TrayOpenSessionGeometry` freezes that viewport for the open generation. The root ignores the native container's horizontal safe area and applies exactly one symmetric 16-point inset. Empty idle content is non-scrolling; all nonempty/transient overflow uses one generation-keyed `ScrollView` with a top default and size-based bounce.
...
| Display | Extended/mirrored output, primary display, per-display resolution/refresh | ColorSync ICC profile, raw runtime ID, x/y origin, rotation, active state, HDR/pixel encoding/vendor modes |
