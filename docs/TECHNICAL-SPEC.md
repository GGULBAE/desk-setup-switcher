...
Display and Audio use a stable two-step editor with seven directly visible kinds: primary display, per-display resolution, output device/volume/mute, and input device/volume. `VisibleSettingRegistry` requires all seven kinds. `ProfileEditorWorkspaceLayoutPolicy` uses only width and accessibility text to select rail versus segmented navigation. There is no Advanced disclosure or Network editor route. `ProfileApplicabilityNormalizer` keeps output mute applicable while disabling mirroring, ColorSync, legacy Network/Input-adapter leaves, and other retired options at decode/store/plan boundaries without deleting their values. `LiveAdapterFactory` constructs only Display/Audio, and Capture bypasses Location permission. Display snapshot modes use zero refresh preference; `DisplayResolutionMatcher` deduplicates dimensions and resolves both old and new profile resolutions against the current session rate. It skips unavailable combinations rather than selecting another rate. The topology transaction retains complete current-state preflight and rollback; primary changes translate origins as required, while relative arrangement and mirroring are not profile requests. Device catalogs and per-device writable-volume/mute checks remain runtime-only. Last-application metadata remains stored but is not rendered. Older adapter details below are compatibility/mock primitives, not extra current profile settings.
