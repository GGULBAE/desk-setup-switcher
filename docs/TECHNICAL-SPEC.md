...
The current stable permission-handoff tree passed integrated non-live `make verify` on 2026-07-14 with 301 default cases: 129 XCTest cases and 172 Swift Testing cases, with six opt-in skips and zero failures. Across both frameworks, five skips are read-only hardware cases and one is a Keychain-write case. Lint/localization policy, Swift Debug/Release, universal Xcode Debug/Release, Analyze, package/checksum, mounted metadata/resources/architectures, and ad-hoc signature classification passed. Separately, `git diff --check` passed. The pre-handoff capture-permission implementation commit is `b34dfd7`; no push or new CI run was performed.
