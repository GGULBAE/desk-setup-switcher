...
Canonical commands are `build`, `test`, `lint`, `analyze`, `package`, `verify-package`, `verify`, and `clean` through the checked-in Makefile. `verify` runs format/lint and source-policy checks, tests, Swift and universal Xcode Debug/Release builds, Xcode Analyze, DMG creation, checksum validation, mounted-DMG inspection, signature classification, and `git diff --check`. The local integrated gate passes. CI performs no live discovery or system mutation; the pushed Actions run remains pending.

Packaging builds with automatic signing disabled, then ad-hoc signs the staged app. The verifier requires `Signature=adhoc`, no identity authority, and a structurally valid signature. The final universal DMG SHA-256 is `3f99ebcea13ea1495e9c2471a45f66dacb851e3ba6670ce16aa84f48b26b99b7`. This is code integrity only: the app has no Developer ID identity, no notarization, and no verified Gatekeeper trust path.
