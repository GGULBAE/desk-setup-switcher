...
Canonical commands are `build`, `test`, `lint`, `analyze`, `package`, `verify-package`, `verify`, and `clean` through the checked-in Makefile. `verify` runs format/lint and source-policy checks, tests, Swift and universal Xcode Debug/Release builds, Xcode Analyze, DMG creation, checksum validation, mounted-DMG inspection, signature classification, and `git diff --check`. CI performs no live discovery or system mutation. Milestone `0d8f510` was pushed, but Actions run `29154880831` failed in `make verify` under Xcode 16.4/Swift 6.1.2 when the `NetworkSystemAPI` `??` autoclosure operated over actor-isolated `[String: Any]`. The local repair extracts Sendable values first, and post-fix full `make verify` passes; repaired push and green retry remain pending.

Packaging builds with automatic signing disabled, then ad-hoc signs the staged app. The verifier requires `Signature=adhoc`, no identity authority, and a structurally valid signature. The post-fix universal DMG SHA-256 is `246af7c21ac9f1ffd4c6f7523f857737f148e4354a948b0e4d9a2123bb5d827f`. This is code integrity only: the app has no Developer ID identity, no notarization, and no verified Gatekeeper trust path.
