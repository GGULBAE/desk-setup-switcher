...
The release-only scripts additionally require GitHub's `RUNNER_ENVIRONMENT=github-hosted`. Canonical secret-bearing steps use direct `/bin/bash` exec, copy each secret to a deliberately non-exported variable, and unset its workflow environment name before any child command. Private randomized certificate/Keychain/notary paths live below `RUNNER_TEMP`; only non-secret signing path names pass to later steps through `GITHUB_ENV`. Each decoded credential is removed at the earliest safe point with an absence check. Notarization runs in a tracked process group; catchable `INT`/`TERM` uses a bounded `TERM` grace period, escalates the group to `KILL`, and enters status-preserving cleanup plus an idempotent always-cleanup. `SIGKILL` delivered directly to the runner shell or runner loss necessarily falls back to GitHub-hosted runner teardown, and `security import -P` briefly carries the certificate passphrase in that isolated process argument rather than a child environment. Only an attempt-1 `build-candidate` dispatch builds and signs; its exact nine files are retained in an immutable origin-run artifact before draft mutation. A separate `prepare-draft` dispatch binds the origin run/artifact ID/archive SHA-256, verifies the raw archive and all attestations before mutation, preserves the manifest's origin run and attempt, and uses a fail-closed additive-only draft reconciler that rejects conflicting state and never clobbers, edits, deletes, publishes, or moves a tag. Draft-run retries remain separate from the origin artifact; rerunning the origin build is forbidden because GitHub full reruns remove that artifact.
