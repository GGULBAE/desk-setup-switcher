...
2. Write a uniquely named sibling staging file and set its mode to `0600` before commit.
3. Atomically replace the destination with one same-filesystem `rename`; no fallible permission operation occurs after replacement, and no stronger sudden-power-loss durability guarantee is currently claimed.
