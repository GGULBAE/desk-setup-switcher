...
`ProfileApplicabilityNormalizer` applies the current surface/support policy without changing the schema or deleting values. It forces legacy activation to true; recognizes key repeat interval, initial repeat delay, and keyboard brightness as the current Input leaves while preserving their existing inclusion flags; and excludes pointer speed, natural scrolling, standard-function-key behavior, display origin/rotation/active state, audio system-output, Wi‑Fi power/SSID, global IPv4, DNS, and proxy leaves. A new Capture includes readable Keyboard values and an explicit editor change opts a leaf in; decode/normalization alone never reactivates a formerly dormant excluded value. Group inclusion derives from current included leaves. Output mute remains an applicable visible Audio leaf. The transform is idempotent and runs at codec decode, local-store load/update, import, snapshot construction, and engine preparation boundaries. Thus older local or imported data cannot re-enable still-dormant mutations, while round trips retain their values. A missing additive `keyboardBrightness` key decodes excluded with no invented value.

Supported controls use typed ephemeral catalogs in `AdapterSnapshot`: display modes, Core Audio devices and writable volume/mute controls, a runtime Keyboard brightness capability, and exact network IPv4 rollback entries. The editor projects a control only when its capability evidence proves the complete vertical slice. Catalog entries, runtime URLs, CoreHID device/element handles, display IDs, service IDs, and exact rollback dictionaries are not part of `DeskProfile` JSON.
...
2. Open and verify the owner-controlled parent directory once, then remove and absence-verify extended ACLs on managed directories/files, create a short uniquely named sibling staging file, remove any inherited ACL, enforce `0600`, write, and synchronize it relative to that directory descriptor.
3. Revalidate the parent path and destination identity, then use `RENAME_EXCL` for an initially missing destination or `RENAME_SWAP` for an existing regular file. Verify the committed identities and perform only no-follow, identity-guarded rollback/cleanup relative to the same descriptor. There is no path-based mutation fallback.
...
Darwin does not expose inode-conditional rename or unlink. The descriptor anchor, exclusive/swap operations, post-checks, and guarded rollback close the ancestor-redirection class and narrow leaf races, but cannot provide a kernel-level compare-and-swap guarantee against a noncooperative process running as the same UID. Replacing an existing leaf requires a swap followed by a separate unlink; abrupt process or power loss between them can leave the former private `0600` file under its randomized staging name. Parent-directory sync is best effort, so perfect sudden-power-loss durability is not claimed.

Export uses a user-selected destination and a distinct no-overwrite mode of the atomic writer. The selected parent is opened and identity-bound only when it is current-user-owned without shared POSIX writes, current-user-owned and sticky, or root-owned and sticky such as macOS `/tmp`; other-owner, non-sticky shared-write, and any extended-ACL ALLOW parent fails closed, while deny-only ACLs remain compatible. The short randomized sibling staging file has inherited ACLs removed and absence-verified before `0600` enforcement, is identity-revalidated immediately before commit, and does not inherit the final leaf's possible `NAME_MAX` length. Complete encoded bytes are synchronized before `RENAME_EXCL`, so any existing directory entry wins without cooperative replacement. Tests cover ALLOW-ACL rejection and inherited-ACL removal, commit-boundary destination insertion, existing regular/special entries, parent/staging replacement, maximum-length leaves, and injected failures before and after commit. The cooperative path publishes only complete bytes; a noncooperative same-UID process retains the documented final adjacent-syscall race. An abrupt stop before rename can leave a private staging residue; a detected staging replacement is deliberately not deleted; parent-directory sync remains best effort. Import reads only the selected file and passes the same size, schema, and semantic validation. Secrets use Keychain; profiles and backups never contain passwords.
...
Display, Audio, and Input use a stable three-step editor with ten directly visible kinds: primary display, per-display resolution, output device/volume/mute, input device/volume, key repeat speed, repeat delay, and keyboard brightness. `VisibleSettingRegistry` requires all ten kinds. `ProfileEditorWorkspaceLayoutPolicy` uses only width and accessibility text to select rail versus segmented navigation. There is no Advanced disclosure or Network editor route. `ProfileApplicabilityNormalizer` keeps output mute and the three Keyboard leaves applicable while disabling mirroring, ColorSync, legacy Network leaves, pointer speed, natural scrolling, standard-function-key behavior, and other retired options at decode/store/plan boundaries without deleting their values. `LiveAdapterFactory` constructs Display, Audio, and Input adapters, and Capture bypasses Location permission. The Input adapter uses experimental undocumented CFPreferences keys for repeat values. On macOS 15 or later its public-CoreHID brightness boundary first checks `IOHIDCheckAccess(kIOHIDRequestTypeListenEvent)` without requesting access, then projects a value only after discovering exactly one compatible element from a built-in HID device that is readable and writable; unsupported, denied, unreadable/unwritable, and ambiguous results are nonfatal. Display snapshot modes use zero refresh preference; `DisplayResolutionMatcher` deduplicates dimensions and resolves both old and new profile resolutions against the current session rate. It skips unavailable combinations rather than selecting another rate. The topology transaction retains complete current-state preflight and rollback; primary changes translate origins as required, while relative arrangement and mirroring are not profile requests. Device catalogs, CoreHID handles, and per-device writable controls remain runtime-only. Last-application metadata remains stored but is not rendered. Older adapter details below are compatibility/mock primitives, not extra current profile settings.
...
One primary menu slot selects normal mode for a Ready profile with operations and available-items/force mode for a Partial profile with executable operations. Normal mode rejects a plan with any included applicable unavailable item. Force mode records unavailable changed items as omissions and proceeds only after explicit UI confirmation. A plan with zero applicable operations is rejected in both modes with a distinct already-matches or no-available-work reason. Adapter no-op comparison occurs before capability/rollback omissions so an already-satisfied included value cannot create a false partial state. A usable cached action can remain enabled during refresh, but transaction/protected-change locks and per-profile preparation always block execution. No-op comparisons are adapter-specific and tolerate insignificant numeric variance.
...
Each planned operation declares risk and fatal-on-failure behavior. The engine orders the stable groups as dormant input, Audio, Display, then Network, so protected rollback reverses connectivity before display. Current profiles request only atomic topology within Display; ColorSync leaves are normalized out before adapter planning. There is no general dependency graph in schema version 1.

A display topology operation first uses Core Graphics' app-only configure scope. Network IPv4 operations join the same protected transaction token, and the app presents a 15-second prompt. **Keep Changes** calls adapter confirmation hooks; timeout, Revert, red-close, termination, or failed confirmation restores completed protected operations in reverse order. Permanent display commit is not used before Keep. These paths are mock verified only.
...
The experimental Input adapter snapshots immediately after synchronizing a repeat-preference write or setting a CoreHID brightness value. A value mismatch returns a failed item rather than success. Rollback carries only the typed prior value captured during planning; runtime CoreHID handles are resolved again and never persisted. Tests inject preference and CoreHID read-back and are deterministic; no live Input mutation has been run.
...
- Keyboard: repeat values are global preferences, while brightness resolves a runtime public-CoreHID element only when discovery produces exactly one compatible readable/writable match from a built-in HID device. CoreHID device and element handles are session-only and never persistent identity.
...
`LSUIElement` requests an accessory/menu-bar lifecycle. Production owns one `NSStatusItem`, one `.applicationDefined` `NSPopover`, and one persistent `NSHostingController`; SwiftUI supplies contained tray content but does not own the surface lifecycle. `TrayGeometry` selects a 368-point-wide viewport. One-to-three-profile content is measured once in a detached, non-interactive hosting view before opening, using the same localized content without its ScrollView or scroll/focus tasks. The resulting height is rounded up and clamped to the active screen and 560-point maximum. The 260/260/316/480/560-point count tiers remain fallbacks for 0/1/2/3/4+ profiles when fitting is unavailable, with empty/overflow policy unchanged. `TrayOpenSessionGeometry` freezes the selected viewport for the open generation; attached content never feeds back another size. The root ignores the native container's horizontal safe area and applies exactly one symmetric 16-point inset. Empty idle content is non-scrolling; all nonempty/transient overflow uses one generation-keyed `ScrollView` with a top default and size-based bounce.
...
App-lifetime controllers own deletion/focus intent, capture phase/task, permission task, dirty-draft decisions, apply preview/safety/result work, and destination errors. Popover disappearance does not cancel or duplicate these operations. Persistent `NSWindowController` + `NSHostingController` surfaces handle Settings, permission, dirty capture/apply, preview, safety confirmation, and result details. Settings retains its 680×480 minimum, one 210-point-sidebar/390-point-minimum-editor horizontal anatomy, stable footer/action bars, and shared navigation/editor objects. A presentation generation resets scroll only when a hidden window reopens; profile identity resets it when selection changes. The runtime catalog remains unchanged while a read-only refresh is in flight, adopts at most its first completed refresh in that presentation, and remains fixed until a true reopen. Workflow child screens fill one persistent canvas; each new apply request supplies a new top-reset identity. Apply Preview starts at 620×440 with a 520×360 minimum and keeps one ordered review-to-decision scroll sequence. It leads with localized change/skip/review counts and compact before → after operation cards, then keeps omissions, validation, and rejections inside one disclosure; a blocking plan expands that disclosure automatically. The localized Beta hardware-verification status, refreshed-plan warning, protected-change timer, and final decision notice remain visible outside ordinary collapsed details. At the accessibility-text minimum, the plan appears before the decision actions; Escape remains the cancel shortcut and Apply has no default Return shortcut.
...
- Core/system unit and mock integration: applicability normalization/idempotence and store/import boundaries, additive Keyboard schema compatibility, supported display-mode catalog capture, repeat and CoreHID brightness capability/snapshot/plan/apply/read-back/rollback, nonfatal unsupported/permission/ambiguous brightness discovery, immediate Input read-back disagreement, success, partial available-items apply, dormant-condition manual planning, fatal failure and reverse rollback, rollback failure, missing device, corrupt storage, and import/export.
...
On 2026-07-20, `DESK_SETUP_LIVE_READ_TESTS=1 make test` passed all 506 then-current app cases on an Apple M5 Mac running macOS 26.5.2. Display, Audio, Network, Input, ConditionContext, and ApplyLivePreparation group/base paths ran read-only. The display gate accepted the legitimate zero-active state while an online session display slept, verified a typed nonfatal empty snapshot in that state, and retained full count, identity, mode, and item assertions whenever active displays existed. The dated run did not itemize actual ColorSync-profile, input-volume, or exact service-IPv4 field presence/read on that host, so those item-level claims and every apply/rollback path remain mock-only. The historical fresh install created one schema-v1 Ready profile from a read-only snapshot with all four groups, and its zero-operation plan kept Apply and Force Apply disabled. There is no live mutation, hardware rollback, live Keychain write, physical Intel, latest permission-flow screenshot, complete keyboard traversal/focused-control AX observation, or TCC matrix evidence. Signing/notarization and release publication were not performed.

## Static and release verification

The preceding UI-stability follow-up passed 375 default non-live cases: 134 XCTest cases with five skipped opt-in cases and 241 Swift Testing cases with two disabled opt-in cases. Lint/localization policy, Swift Debug/Release, universal Xcode Debug/Release, Xcode Analyze, package/checksum creation, and mounted DMG metadata/resource/architecture/signature verification passed. Its historical no-Developer-ID DMG SHA-256 is `516b968718aeb3c1c247e1a2deca7a28d45820d119f31ea992d4b475071f3638`. No live mutation flag was set. That app replaced the `/Applications` bundle and passed read-only identity/version/architecture/signature/executable-equality checks; it was not launched after replacement.

The historical settings-lifecycle/UI-declutter refactor passed 401 default non-live cases: 144 XCTest cases with five skipped opt-in cases and 257 Swift Testing cases with two default-disabled opt-in cases. Lint/localization policy, Swift Debug/Release, universal Xcode Debug/Release, Xcode Analyze, package/checksum creation, and mounted DMG metadata/resource/architecture/signature verification passed. Its historical no-Developer-ID DMG SHA-256 is `f3aa610026179161208dec2cb2ef6185768843becd8e0e56bccc9f8abab37f2b`. No live flag was set, and the package was not installed or launched.

Canonical commands are `build`, `test`, `lint`, `analyze`, `package`, `verify-package`, `verify`, and `clean` through the checked-in Makefile. `lint` includes source-policy checks and English/Korean key-parity, duplicate-key, format-placeholder, and static-localization-key validation. `verify` runs format/lint, app tests, the deterministic release-policy/guard suite, Swift and universal Xcode Debug/Release builds, Xcode Analyze, DMG creation, checksum validation, mounted-DMG inspection, ad-hoc signature classification, and the stricter release app-bundle verifier that rejects unexpected executables or Mach-O files. The local closure gate runs `git diff --check` separately. CI performs no live discovery or system mutation. Initial run `29154880831` for `0d8f510` exposed the Swift 6.1 actor-isolation issue, and repair [run `29155207923`](https://github.com/GGULBAE/desk-setup-switcher/actions/runs/29155207923) remains historical compatibility evidence. UI-hardening commit `5f0cabc` passed full `make verify` and unsigned-package upload in [run `29181900967`](https://github.com/GGULBAE/desk-setup-switcher/actions/runs/29181900967). The current local workflow definition is unpushed; no current remote CI result is claimed.

The release-only scripts additionally require GitHub's `RUNNER_ENVIRONMENT=github-hosted`. Canonical secret-bearing steps use direct `/bin/bash` exec, copy each secret to a deliberately non-exported variable, and unset its workflow environment name before any child command. Private randomized certificate/Keychain/notary paths live below `RUNNER_TEMP`; only non-secret signing path names pass to later steps through `GITHUB_ENV`. Each decoded credential is removed at the earliest safe point with an absence check. Notarization runs in a tracked process group; catchable `INT`/`TERM` uses a bounded `TERM` grace period, escalates the group to `KILL`, and enters status-preserving cleanup plus an idempotent always-cleanup. `SIGKILL` delivered directly to the runner shell or runner loss necessarily falls back to GitHub-hosted runner teardown, and `security import -P` briefly carries the certificate passphrase in that isolated process argument rather than a child environment. The four-workflow release contract retains distinct attempt-1 exact-nine-asset origins for protected `v0.0.9`/build 1 and `v0.1.0`/build 2. A separate `prepare-draft` dispatch binds only the final origin run/artifact ID/archive SHA-256, verifies its raw archive and attestations before mutation, preserves the manifest's origin run and attempt, and uses a fail-closed additive-only draft reconciler that rejects conflicting state and never clobbers, edits, deletes, publishes, or moves a tag. The predecessor creates no Release. Draft-run retries remain separate from both origin artifacts; rerunning either origin build is forbidden because GitHub full reruns remove that artifact.
...
Packaging builds with automatic signing disabled, then ad-hoc signs the staged app. The verifier requires `Signature=adhoc`, no identity authority, a structurally valid signature, no unexpected executable or Mach-O files, exactly `arm64+x86_64`, and macOS 14.0 in each slice's `LC_BUILD_VERSION`. The current `v0.0.9`/build-1 universal `artifacts/Desk-Setup-Switcher-0.0.9-unsigned.dmg` passed the mounted verification gate with SHA-256 `6a77ad1f830f30faa620a1e6f29ab1f9fb035c43ad43b9e6426ddcc58b355cff`; its final ad-hoc-signed mounted and installed executable SHA-256 is `6d67115a107fb35d098b6d8640cadb025a1e60d716235f5ae618b82614411d0a`. It remains local development evidence and was not uploaded or published. Earlier local and installed-interaction checksums remain historical in the completion ledger; downloaded CI artifact ID `8256718472` verified the earlier UI-hardening package at SHA-256 `f3d82b033e8e375c9063a9b72cbd174d94a03f0cdd4414961895db3b3dcfc3f4`. The DMGs are not byte-for-byte reproducible. These signatures provide code integrity only: the app has no Developer ID identity, no notarization, and no verified Gatekeeper trust path.
...
- `trayDidOpen` creates the open generation and a new top-default scroll identity; after `NSPopover.show`, `trayContentDidAttach` publishes the guarded top-scroll request. The root attaches the anchor ID directly to its first content block, so no anchor-only sibling contributes stack spacing.
...
- Status-item title/width changes are deduplicated and deferred for the complete visible generation because that button is also the popover anchor.
...
| Display | Primary display, per-display resolution (preserves current refresh rate) | Refresh-rate preference, mirroring, ColorSync ICC profile, raw runtime ID, x/y origin, rotation, active state, HDR/pixel encoding/vendor modes |
| Audio | Default input, default output, input volume, output volume, output mute | System output UID |
| Network | None | All legacy network values, including service IPv4, are dormant; default capture never queries the network adapter |
| Input / Keyboard | Key repeat speed, repeat delay, keyboard brightness when its runtime capability is available | Pointer speed, natural scrolling, standard-function-key behavior, CoreHID handles |
...
`outputMuted` is `SettingOption<Bool?>`. Missing legacy JSON still decodes as `.excluded(nil)`. Snapshot reads the public Core Audio output mute property for the default output UID and separately catalogs every connected output device's readable/settable mute capability. Planning resolves the profile's eventual output UID, stores the prior Boolean as rollback, applies and reads back the desired value, and treats an unsupported or non-settable changed target as an item-level omission. Volume and mute values remain independently nullable and independently planned; the current UI exposes no participation switches.

### Service IPv4 and legacy ColorSync compatibility

`NetworkServiceIPv4Settings` persists `NetworkServiceIdentity(kind, serviceName, interfaceType)` plus DHCP or manual configuration. The snapshot also carries the exact serialized IPv4 protocol dictionary for rollback, but that data remains session-only. No current editor row or Apply operation is projected for this data. Retained legacy adapter tests cover the earlier contract: a row required one exact portable-identity match plus rollback evidence, and Apply used Authorization Services with `SCPreferencesCreateWithAuthorization`, exact service/protocol re-resolution, lock/set/commit/apply/unlock checks, dynamic-store completion, and exact DHCP/manual read-back. Authorization denial, timeout, mismatch, ambiguity, disappearance, and every preferences failure remain typed and non-secret compatibility behavior.

ColorSync is no longer a capture/edit/apply option. Legacy `ColorSyncProfileTarget` data retains registered profile ID, SHA-256 of ICC file bytes, and display name, but is always normalized to excluded. The following lower-level adapter primitives remain only for compatibility/regression coverage and are not requested by the current profile workflow. Runtime file URLs exist only in the current public ColorSync catalog. The adapter resolves exactly one ID/hash pair for the display, captures the existing custom-profile mapping, applies the new mapping, reads it back, and rolls the exact mapping back on failure or Revert. Display topology applies before color; rollback reverses that order. This contract is ICC profile selection only, not a claim about HDR, pixel encoding, vendor presets, or arbitrary “color mode.”
