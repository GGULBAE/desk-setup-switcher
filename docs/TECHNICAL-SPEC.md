...
Display and Audio use a stable two-step editor with six directly visible kinds: primary display, per-display resolution, output device/volume, and input device/volume. `VisibleSettingRegistry` requires all seven vertical stages for those kinds. `ProfileEditorWorkspaceLayoutPolicy` uses only width and accessibility text to select rail versus segmented navigation. There is no Advanced disclosure or Network editor route. `ProfileApplicabilityNormalizer` disables mirroring, mute, ColorSync, all network/input leaves, and other retired options at decode/store/plan boundaries while preserving their values. `LiveAdapterFactory` constructs only Display/Audio, and Capture bypasses Location permission. Display snapshot modes use zero refresh preference; `DisplayResolutionMatcher` deduplicates dimensions and resolves both old and new profile resolutions against the current session rate. It skips unavailable combinations rather than selecting another rate. The topology transaction retains complete current-state preflight and rollback; primary changes translate origins as required, while relative arrangement and mirroring are not profile requests. Device catalogs and per-device writable-volume checks remain runtime-only. Last-application metadata remains stored but is not rendered. Older adapter details below are compatibility/mock primitives, not extra current profile settings.
...
| Audio | Default input, default output, input volume, output volume | System output UID, output mute |
