...
Display, Audio, and Input share one vertically scrolling editor with nine directly visible kinds: primary display, per-display resolution, output device/volume/mute, input device/volume, key repeat speed, and repeat delay. `VisibleSettingRegistry` requires all nine kinds. There is no internal rail, segmented selector, Advanced disclosure, or Network editor route. `ProfileApplicabilityNormalizer` keeps output mute and the two repeat leaves applicable while disabling keyboard brightness, mirroring, ColorSync, legacy Network leaves, pointer speed, natural scrolling, standard-function-key behavior, and other retired options at decode/store/plan boundaries without deleting their values. `LiveAdapterFactory` constructs Display, Audio, and Input adapters, and Capture bypasses Location and Input Monitoring permission. The Input adapter uses experimental undocumented CFPreferences keys for repeat values and has no keyboard-brightness runtime path. Display snapshot modes use zero refresh preference; `DisplayResolutionMatcher` deduplicates dimensions and resolves both old and new profile resolutions against the current session rate. It skips unavailable combinations rather than selecting another rate. The topology transaction retains complete current-state preflight and rollback; primary changes translate origins as required, while relative arrangement and mirroring are not profile requests. Device catalogs and per-device writable controls remain runtime-only. Last-application metadata remains stored but is not rendered. Older adapter details below are compatibility/mock primitives, not extra current profile settings.
...
The experimental Input adapter snapshots immediately after synchronizing a repeat-preference write. A value mismatch returns a failed item rather than success. Rollback carries only the typed prior value captured during planning. Tests inject preference read-back and are deterministic; no live Input mutation has been run.
...
- Keyboard: repeat values are global preferences. Keyboard brightness remains a dormant schema value and has no runtime identity or adapter path.
...
- Core/system unit and mock integration: applicability normalization/idempotence and store/import boundaries, additive Keyboard schema compatibility with brightness forced dormant, supported display-mode catalog capture, repeat capability/snapshot/plan/apply/read-back/rollback, crafted legacy-brightness operation rejection, immediate Input read-back disagreement, success, partial available-items apply, dormant-condition manual planning, fatal failure and reverse rollback, rollback failure, missing device, corrupt storage, and import/export.
...
| Input / Keyboard | Key repeat speed, repeat delay | Keyboard brightness, pointer speed, natural scrolling, standard-function-key behavior |
