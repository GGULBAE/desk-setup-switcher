...
A separately authorized reinstall replaced `/Applications/Desk Setup Switcher.app` with that mounted package and launched the installed path. The mounted and installed executables matched at SHA-256 `f6accf8edc908a426f0082bcba1b11807f7f1bf8f862033ecd56c7e6c8d7cdcc`; bundle identity, ad-hoc signature, and `x86_64 arm64` slices passed read-only checks. Profile primary and backup hashes remained unchanged, and the previous app plus preinstall profile copies remain in a recoverable build-local backup. No Settings interaction, Capture, Apply, TCC, login-item, Keychain, display, ColorSync, audio, network, mouse, or keyboard mutation was performed.
