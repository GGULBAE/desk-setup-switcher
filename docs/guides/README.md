...
- The current profile has nine Display, Sound, and Keyboard settings: main display, resolution, output device/volume/mute, input device/volume, key repeat speed, and repeat delay. Repeat values are experimental. Keyboard brightness, pointer, scrolling, function-key, and Network values remain dormant and are never captured or applied.
...
- 현재 프로필은 Display·Sound·Keyboard의 아홉 가지 설정(주 디스플레이, 해상도, 출력 기기·음량·음소거, 입력 기기·음량, 키 반복 속도, 반복 지연 시간)을 제공. 반복 값은 실험적이며 키보드 밝기·포인터·스크롤·기능 키·Network 값은 비활성 상태로 보존하고 캡처하거나 적용하지 않음
