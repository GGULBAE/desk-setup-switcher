...
The current 2026-07-20 local release-controls candidate's no-Developer-ID path passed 3,199 deterministic checks/assertions: 506 app checks (183 XCTest cases, 322 default Swift Testing cases across 39 suites, and one isolated native `NSPopover` case in a 40th Swift Testing suite) plus 2,693 release-tooling assertions (328 base release-policy, 659 remote-controls v1 policy/normalizer, 178 remote-controls v2 lifecycle-policy, 69 remote-controls v2 collector, 234 publication-approval policy, 129 collector-wrapper mock, 71 draft-reconciler mock, 246 artifact-restoration mock, 424 approved-publication mock, 152 legacy-workflow-containment mock, and 203 shell/workflow guard assertions). Universal Debug/Release, Analyze, project-generation verification, DMG creation, SHA-256 validation, mounted metadata/resources, `arm64 x86_64`, and ad-hoc signature classification passed. The release-tooling evidence is simulated and structural, not a credentialed signing, notarization, protected-remote, or publication result. The package was not installed or launched.

- Current development-only DMG SHA-256: `6cd2a3d512c9656d71fa916d3a3a0460894f8351d335fb7217ded4920b808805`
...
Local containment tooling is now prepared, but has not been used against GitHub. With `GH_TOKEN` present, `make plan-legacy-workflow-containment EXPECTED_MASTER_SHA=<exact remote master> CONTAINMENT_PLAN_RECEIPT=/absolute/private/plan.json` performs only two complete GET-only observations and creates one owner-only mode-0600 receipt outside the repository under an owner-mode-0700 directory. It binds the exact repository/actor/admin identity, `master`, workflow ID/name/path/state, unsafe workflow blob, empty `v*` tag/Release boundary, completed workflow-run set, local tool blobs, and paginated observation digests.
