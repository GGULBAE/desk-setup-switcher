...
| Build number | `CFBundleVersion = 2` | Must be a positive canonical decimal integer and increase across every new Developer ID candidate, even when the app version changes. Restoring the exact origin artifact is not a new candidate. |
...
The fixed first-public-beta lineage reserves `v0.0.9` with `CFBundleVersion = 1` as a protected-beta predecessor and `v0.1.0` with `CFBundleVersion = 2` as the final public-beta candidate. `v0.0.9` is not a supported public release and must never receive a GitHub Release; it exists only as a separately approved, signed/notarized, retained upgrade source. Publication requires the exact build-1 predecessor to upgrade into the exact build-2 final candidate.

Every protected Developer ID candidate dispatch consumes its build number, including a failed, cancelled, or abandoned candidate. A rebuild, new signing dispatch, changed source/resources/entitlements, or unavailable origin artifact requires a number greater than every recorded candidate. Gaps are allowed in the general rule; reuse is not. The fixed first-beta policy admits only the build-1 predecessor and build-2 final candidate. Only byte-for-byte restoration of the same origin run/artifact may retain a number. The release approval gate reads both exact candidate manifests plus the actual-byte [candidate inventory and predecessor lineage](PREDECESSOR-LINEAGE.md); build `0`, leading zeroes, signs, whitespace, and dotted values are rejected. The offline validator checks ordering, uniqueness, identity, and byte bindings; exhaustive remote-history review remains a protected approver responsibility.

## CPU support

The Xcode and package gates currently cross-build a universal `arm64 x86_64` executable. The release verifier rejects missing or extra slices and requires each slice's `LC_BUILD_VERSION` minimum to equal macOS 14.0; it records the same check after signing and again from the mounted final DMG, binds both records to the executable SHA-256, and independently matches their minimum to this repository policy. Runtime and install evidence exists on Apple Silicon. Physical Intel execution, clean install, upgrade, profile recovery, and mutation/rollback have not been verified.
