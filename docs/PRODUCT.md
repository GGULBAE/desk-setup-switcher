...
The editor presents one stable, numbered **Display**, **Sound**, or **Network** step at a time. Normal windows keep the step rail visible; minimum-width and accessibility-text layouts use a segmented selector. Each step leads with one compact card for the common task: display arrangement, sound output, or one selected network connection. Resolution/refresh/ColorSync, audio input/mute, and manual IPv4 details remain fully editable behind a per-step **Advanced settings** disclosure. Closing that disclosure never deletes included values, and validation opens the owning step and disclosure before focusing a hidden invalid field. Selecting a network connection is presentation state only; its nearby inclusion switch changes that service without disabling another included service. Runtime catalogs from read-only adapter snapshots are never stored in profile JSON. Unsupported runtime choices remain absent, but a persisted included Audio, ColorSync, or service-IPv4 target that later becomes unavailable remains visible in Advanced settings as a non-color warning with an inclusion-off repair control. Display rotation/active state and administrative IPv4, DNS, and proxy values are retained only as read-only snapshot evidence. An idempotent normalizer excludes these leaves—and any group left without an applicable leaf—when capturing, loading, decoding, planning, or importing without deleting the stored values. Conditions remain round-trip-compatible profile data but are dormant for current manual readiness and Apply. The 2026-07-11 fresh-install smoke test predates this behavior and is not a current UI walkthrough.
