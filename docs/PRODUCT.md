...
The visible surface is limited to ten settings across Display, Sound, and Keyboard: main display, resolution, output device/volume/mute, input device/volume, key repeat speed, repeat delay, and keyboard brightness. Pointer speed, natural scrolling, standard-function-key behavior, other historical settings, and imported conditions remain format-compatible dormant data, never automatic triggers or hidden profile changes.
...
The editor presents stable numbered **Display**, **Sound**, and **Keyboard** steps. Minimum/accessibility layouts retain the compact segmented selector. All ten setting kinds appear directly when their capability is available, without Advanced or Network. Keyboard shows exactly key repeat speed, repeat delay, and keyboard brightness as semantic, discrete Slow → Fast, Long → Short, and Dim → Bright sliders rather than raw preference fields; it never exposes pointer speed, natural scrolling, or standard-function-key behavior. Native repeat Off is not the slowest enabled speed and remains unsupported until its separate runtime state can satisfy the same capture/apply/read-back/rollback contract. There are no per-setting inclusion switches. A new Capture includes each readable Keyboard value, and explicitly editing one opts it in; upgrading preserves a formerly dormant leaf's exclusion instead of silently activating its saved value. Missing or unavailable values remain absent. Validation selects the owning step before moving focus; unavailable saved audio or keyboard values show a warning that directs the user to choose another device or review available settings. Runtime catalogs and CoreHID element handles are never stored in profile JSON. Normalization excludes every retired leaf at capture/load/import/plan boundaries without deleting old values, including in Force mode. Last-application history is not shown. The current ten-setting scope is recorded in the 2026-09-15 Keyboard milestone; the [minimal-profile note](MINIMAL-PROFILE-SCOPE-2026-09-07.md) and [output-mute follow-up](OUTPUT-MUTE-2026-09-08.md) remain historical evidence for the earlier surface.
...
Capture queries only Display, Audio, and Input adapters and does not request or gate on Location permission. The Input adapter contributes only the three current Keyboard values; unavailable brightness is omitted without failing the other groups. A dirty draft still requires Save/Discard/Cancel before creating a separate captured profile. Results remain value-free and report applicable captured fields; an empty applicable capture creates no profile. Disabled Apply actions retain text/symbol reasons. Profile summaries and previews use friendly values, with opaque identifiers behind explicit technical disclosures.
...
Capture, editor, and Apply expose exactly key repeat speed, repeat delay, and keyboard brightness. Repeat speed and delay use public CFPreferences access with undocumented preference keys, so their capabilities are experimental and isolated behind the Input adapter contract. Keyboard brightness is experimental on macOS 15 or later and is available only with previously granted macOS Input Monitoring access when public CoreHID discovers exactly one compatible backlight element from a built-in HID device that can be both read and written. Background capability and snapshot work checks `IOHIDCheckAccess(kIOHIDRequestTypeListenEvent)` but never requests access automatically. Unsupported OS or hardware, denial, unreadable or unwritable elements, and ambiguous matches are nonfatal: brightness stays absent or becomes an item-level omission while Display, Sound, and available repeat settings continue.

All three settings use explicit plan, apply, immediate read-back, and rollback semantics. Runtime CoreHID handles are not persistent identity. Deterministic tests use injected adapters and synthetic elements; no live repeat or backlight mutation, physical rollback, or hardware verification is claimed.

### Network

No Network option is part of the current product profile. Capture queries only Display, Audio, and Input adapters and never gates on Location permission. All imported network values, including service-specific DHCP/manual IPv4, remain round-trip-compatible dormant data and are normalized out before planning, even in Force mode. Historical CoreWLAN/SystemConfiguration and condition primitives remain internal compatibility/mock coverage, not current profile capabilities.

### Legacy mouse and keyboard compatibility

The versioned profile format continues to round-trip historical pointer speed, scroll direction, and function-key values, but they are not edited, applied, advertised, or supported in the `v0.1.0` public surface. Key repeat speed and repeat delay have moved into the current Keyboard surface while remaining experimental because their keys are undocumented. Dormant values stay normalized out of both normal and available-items/Force planning.
