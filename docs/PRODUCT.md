...
Desk Setup Switcher is an unreleased `0.1.0` open-source public-beta candidate. The current 2026-07-18 public-surface candidate passed integrated non-live `make verify` with 908 deterministic checks/assertions: 501 app checks (178 XCTest cases, 322 default Swift Testing cases across 39 suites, and one isolated native popover case in a 40th Swift Testing suite) plus 407 release-tooling assertions (328 Ruby policy and 79 shell guard assertions). Lint, Swift and universal Xcode Debug/Release, Analyze, project-generation, and mounted package/checksum verification also passed. The current development-only DMG SHA-256 is `16c8ef3ede9630e15afc9d6627434bcd09d0a2ec4e1c9fc208d5dded9be82de7`; it is universal and ad-hoc signed without Developer ID and was not installed or launched. This is not a public release, mutation proof, or credentialed signing/notarization result. Signing/notarization, quarantined Gatekeeper, clean lifecycle and external beta evidence, Homebrew own-tap verification, physical Intel, live setting mutation, live Keychain write, and the permission matrix remain unverified. Full-app VoiceOver certification is excluded from the release goal and is not claimed. The separate 496-check result remains a historical baseline in [OPEN-SOURCE-RELEASE-BASELINE-2026-07-18.md](OPEN-SOURCE-RELEASE-BASELINE-2026-07-18.md); current status is tracked in [COMPLETION-CRITERIA.md](COMPLETION-CRITERIA.md) and [SUPPORT-MATRIX.md](SUPPORT-MATRIX.md).
