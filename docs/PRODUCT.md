...
The editor presents one stable, numbered **Display**, **Sound**, or **Network** step at a time. Normal windows keep the step rail visible; minimum-width and accessibility-text layouts use a segmented selector. Each step leads with one compact card for the common task: display arrangement, sound output, or one selected network connection. Display resolution/refresh and audio input/mute appear only when Advanced settings is expanded. Network keeps its connection and DHCP/manual IPv4 cards. Opening details or switching sections cannot change the rail layout; only viewport width and accessibility text select the compact layout. The editor does not display last-application history. Validation opens the owning step and selects the matching network connection before focusing an invalid field. Selecting a network connection is presentation state only; its nearby inclusion switch changes that service without disabling another included service. Runtime catalogs from read-only adapter snapshots are never stored in profile JSON. Unsupported runtime choices remain absent, but a persisted included Audio or service-IPv4 target that later becomes unavailable remains visible alongside the related controls as a non-color warning with an inclusion-off repair control. Color profiles are removed from new captures and the editor; legacy color values remain dormant and cannot be requested through profile Apply. Display rotation/active state and administrative IPv4, DNS, and proxy values are retained only as read-only snapshot evidence. An idempotent normalizer excludes these leaves—and any group left without an applicable leaf—when capturing, loading, decoding, planning, or importing without deleting the stored values. Conditions remain round-trip-compatible profile data but are dormant for current manual readiness and Apply. The 2026-07-11 fresh-install smoke test predates this behavior and is not a current UI walkthrough.

### App preferences

**App Settings** contains launch-at-login preferences, Wi-Fi capture access, and an on-demand Diagnostics action. Login registration status and refresh are available inside **Login item details**; mismatches and retry actions stay visible beside the launch preference. These preferences apply to the app, independently of the selected profile.
...
Discover connected displays and stable identity attributes; snapshot primary display, topology, mirroring, mode, refresh rate, rotation, and active state where public Core Graphics APIs expose them. The current editor exposes extended/mirrored output, primary display, and per-display mode; color profiles, origin, rotation, and active state remain dormant. A typed supported-mode catalog travels only with the current read-only snapshot so the editor can offer a safe picker and preserve a saved mode that is temporarily unavailable; the catalog is not profile data. Validate saved modes before applying. Never persist a `CGDirectDisplayID` as the sole identity. Core Graphics topology starts at app-only scope and **Keep Changes** promotes it to session scope. Timeout, revert, or failed confirmation asks the adapter to restore the prior state. The 2026-07-20 Display group/base read-only path passed on Apple Silicon/macOS 26.5.2, but that run did not itemize an actual ColorSync-profile presence/read on the host. That item-level claim and all display apply/rollback paths remain mock-only; no mutation was run.

### Audio

Discover Core Audio input/output devices by UID. The current editor and apply payload expose default input, default output, settable input/output volume, and settable output mute. Volume and mute are independent included values; capture records both when available. System output remains round-trip-compatible dormant data rather than a `v0.1.0` control. Read-only capability catalogs are device-scoped so a profile that switches devices evaluates the eventual target's volume and mute controls. An included change with no writable control is an explicit omission rather than a silent success; an already-matching value remains a no-op. The 2026-07-20 Audio group/base read-only path passed on Apple Silicon/macOS 26.5.2, but that run did not itemize actual input-volume or output-mute field presence/read on the host. Those item-level claims and all audio apply/rollback paths remain mock-only; no mutation was run.

### Network

Use CoreWLAN, Network, and SystemConfiguration for current Wi-Fi, Ethernet/link, interface/IP/subnet/gateway/DNS, and readiness facts. The current editor exposes only an exact portable Ethernet/Wi-Fi service target with DHCP or manual IPv4 when one target and exact rollback evidence resolve; apply uses authorized public SystemConfiguration APIs and a protected rollback window. Wi-Fi power/association and legacy global IPv4, DNS, and proxies are not current `v0.1.0` controls; service order is absent rather than persisted. A powered-on interface whose SSID is unreadable is ambiguous, not assumed disassociated, and no Wi-Fi password enters a profile or log. The 2026-07-20 Network group/base read-only path passed on Apple Silicon/macOS 26.5.2, but that run did not itemize an actual exact service-IPv4 field presence/read on the host. That item-level claim and all network apply/rollback paths remain mock-only; no authorization prompt, service write, or hardware rollback was run.
