...
The editor presents **Display**, **Sound**, and **Keyboard** together in one vertical scroll view without an internal rail or segmented selector. All nine setting kinds appear directly when their capability is available, without Advanced or Network. Keyboard shows exactly key repeat speed and repeat delay as semantic, discrete Slow → Fast and Long → Short sliders rather than raw preference fields; it never exposes keyboard brightness, pointer speed, natural scrolling, or standard-function-key behavior. Native repeat Off is not the slowest enabled speed and remains unsupported until its separate runtime state can satisfy the same capture/apply/read-back/rollback contract. There are no per-setting inclusion switches. A new Capture includes each readable repeat value, and explicitly editing one opts it in; upgrading preserves dormant legacy values without making them actionable. Missing or unavailable values remain absent. Validation scrolls to the owning section before moving focus; unavailable saved audio or keyboard values show a warning that directs the user to choose another device or review available settings. Runtime catalogs are never stored in profile JSON. Normalization excludes every retired leaf, including keyboard brightness, at capture/load/import/plan boundaries without deleting old values, including in Force mode. Last-application history is not shown. The [2026-09-15 Keyboard milestone](KEYBOARD-REINSTALL-2026-09-15.md) records the superseded three-control build; the [minimal-profile note](MINIMAL-PROFILE-SCOPE-2026-09-07.md) and [output-mute follow-up](OUTPUT-MUTE-2026-09-08.md) remain historical evidence for earlier surfaces.
...
Capture, editor, and Apply expose exactly key repeat speed and repeat delay. Both use public CFPreferences access with undocumented preference keys, so their capabilities are experimental and isolated behind the Input adapter contract. Keyboard brightness remains decodable legacy data only and is normalized to inclusion off before capture, editing, planning, or persistence.

Both repeat settings use explicit plan, apply, immediate read-back, and rollback semantics. Deterministic tests use injected preference adapters; no live repeat mutation, physical rollback, or hardware verification is claimed.
