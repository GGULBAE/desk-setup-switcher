...
| Description | “A local-first macOS menu bar app for saving, reviewing, and deliberately applying display and sound profiles.” | This remote value predates Keyboard. A future separately authorized metadata update should use the proposed current description below |
...
Proposed current description (not yet applied remotely):

> A local-first macOS menu bar app for saving, reviewing, and deliberately applying display, sound, and keyboard profiles.
...
Upload only the final sanitized social-preview asset recorded in the release-asset provenance document. The current asset uses synthetic Display/Sound profile data from before the Keyboard surface was added; it must not be described as showing Keyboard. It must contain no real identifier, avoid an “applied successfully” claim, and remain legible when cropped.
...
The companion `site/public/demo/desk-setup-switcher.mp4` is a pre-Keyboard silent 40-second Capture → Display → Sound → Review tour with English and Korean WebVTT captions. Site playback must expose controls, must not autoplay or loop, and should use the Review gallery card as its poster. Its durable transcript remains historical and truthful: capture available Display/Sound values locally, edit the display and sound steps, inspect the proposed changes, then choose separately whether to Apply. Profiles remain local and never switch automatically.
...
- [ ] After applying metadata, a read-only GitHub query confirms the exact description/topics/Homepage and shows no stale current-scope claim for Network, pointer speed, natural scrolling, or standard-function-key behavior; no Intel/Homebrew/accessibility-certification/hardware-verification overclaim; and no claim that the beta is Developer ID-signed or notarized.
