...
The app layer owns compact header actions and a flat, always-expanded Display/Audio/Network editor. There is no profile activation toggle and no group/option disclosure state. Each supported leaf owns its Include switch; unsupported or ambiguous controls are not projected. `VisibleSettingRegistry` is the single contract table for the nine visible setting kinds and requires capture, edit, validation, planning, apply, verification, and rollback metadata for each. Typed runtime catalogs are ephemeral adapter snapshot context and are never persisted as device handles or URLs. Pure validation produces stable field identifiers and typed messages before save.
...
Before planning, `DirtyApplyProtectionDecision` compares the target with the app-lifetime editor session. Save/discard/cancel must resolve explicitly; save failure or cancel ends the request. The normalizer then removes unsupported applicability. Immediately before execution, a fresh profile/snapshot plan must remain execution-equivalent; otherwise no adapter is invoked and the workflow returns to a visibly marked refreshed review. A rejected confirmation also becomes a visible workflow error rather than a silent return.
...
The installed-layout/apply follow-up passed integrated non-live `make verify` on 2026-07-16: localization and source-policy lint; 370 default cases (134 XCTest with five skips plus 236 Swift Testing with two disabled opt-in cases); Swift Debug/Release; universal Xcode Debug/Release; Analyze; package/checksum; mounted metadata/resources/`x86_64 arm64`; and ad-hoc/no-Developer-ID signature classification. `git diff --check` passed separately. The verified DMG SHA-256 is `e2927513543903b794bea08628064b29c12604e18706bef169c20b3e89760bcd`; it replaced the `/Applications` bundle without an automatic launch. Twelve tray and 10 current profile-editor detached PNG/AX pairs plus the injected-safe-area regression cover contained layout; they do not prove the corrected installed tray/window ordering, native localization context, VoiceOver, or hardware behavior.
