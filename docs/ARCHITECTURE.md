...
The default profile editor is a deliberate projection over the full backward-compatible document. It shows metadata name/symbol plus Display, Audio, and Network, including independently includable output mute when the selected output device exposes a writable public control. Description, conditions, Input, display origin/rotation/active, legacy global network values, DNS/proxy, and system-output UID remain decodable and round-trip unchanged but the applicability normalizer excludes them from newly prepared apply payloads. The retired condition-editor source is removed from the app target while its Core schema and compatibility tests remain. Removing a control is therefore not destructive migration and cannot silently re-apply a hidden value.
