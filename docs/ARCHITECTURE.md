...
The tray/settings refinement passed integrated non-live `make verify` on 2026-07-15: localization and source-policy lint; 338 default cases (132 XCTest plus 206 Swift Testing cases, six opt-in skips); Swift Debug/Release; universal Xcode Debug/Release; Analyze; package/checksum; mounted metadata/resources/`x86_64 arm64`; and ad-hoc/no-Developer-ID signature classification. `git diff --check` passed separately. The verified DMG SHA-256 is `539c203607782302799d68acdda2f64666f0ace5897fa325a79e1dfdcfc98f78`. Twelve refreshed tray and five profile-editor detached PNG/AX pairs cover 0/1/3/10 profiles, long Korean, minimum width, accessibility large text, Display, Audio, Network, and dark mode; they do not prove installed tray/window ordering, native localization context, VoiceOver, or hardware behavior.
...
The tray's fixed open-session geometry now has an explicit reopen boundary. Before and immediately after `NSPopover.show`, the AppKit host frame and bounds are reset to a zero origin while retaining the session's fixed content size. A generation-scoped presentation request then scrolls the single SwiftUI viewport to its top anchor. This prevents a previous scroll or hosting-view origin from leaking into a later open without introducing measured-height feedback, nested scrolling, or a corrective magic offset.

Settings and workflow windows are app-lifetime controllers rather than popover-owned destinations. Red-close orders the same window out instead of destroying its root, draft, navigation, or controller. Presentation activates the accessory app, deminiaturizes if necessary, makes the window key/front, and completes exactly once only after visible/key state is observed. The tray router orders the popover out after that completion; cancellation, stale completion, or failure leaves the tray available.
