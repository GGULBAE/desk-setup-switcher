...
Concrete adapters for Core Graphics, Core Audio, CoreWLAN/Network/SystemConfiguration, Input preferences and keyboard backlights, hardware/condition discovery, authorized-location reads, and Keychain. Each setting adapter owns its snapshot-to-operation comparison and rollback data. Core Graphics also supplies a typed ephemeral supported-mode catalog through read-only snapshots; it never enters profile JSON. Repeat preference keys are isolated and reported as experimental, and their writes require immediate adapter read-back agreement. Keyboard brightness uses public CoreHID only on macOS 15 or later, checks Input Monitoring through `IOHIDCheckAccess(kIOHIDRequestTypeListenEvent)` without requesting it during background snapshots, reports an experimental capability only for exactly one compatible readable/writable element from a built-in HID device, and keeps runtime handles out of profiles.
...
flowchart LR
    ...
    Registry --> Input["Keyboard / Input adapter"]
...
The app layer owns compact header actions and a three-step Display/Sound/Keyboard editor. There is no profile activation toggle, per-setting inclusion control, Advanced section, or Network step. Every supported Settings width keeps one horizontal workspace with a fixed sidebar and editor; minimum or accessibility layouts replace the rail with a compact segmented selector, while footer and save/revert bars remain outside scrolling at stable heights. A Settings presentation keeps its typed runtime catalog unchanged while a read-only refresh is in flight, adopts at most the first completed refresh for that presentation, and then ignores incidental or repeated snapshot publications until reopening deliberately adopts the newest snapshot. Reinvoking Settings while its window is already visible does not create a new presentation generation or reset editor focus/scroll. Unsupported or ambiguous controls retain typed availability without becoming unsafe choices or blocking unrelated fields. A persisted unavailable value is preserved and produces a warning or preview omission rather than being guessed or silently deleted. Device-scoped audio volume and mute capability is resolved against the target UID selected by the draft; keyboard brightness is projected only from exactly one compatible readable/writable public-CoreHID element on a built-in HID device on macOS 15 or later. `VisibleSettingRegistry` is the single contract table for the ten visible setting kinds and requires capture, edit, validation, planning, apply, verification, and rollback metadata for each.
...
The default profile editor is a deliberate projection over the full backward-compatible document. It shows metadata name/symbol plus ten settings across Display, Sound, and Keyboard. Keyboard exposes only key repeat speed, repeat delay, and keyboard brightness. A new Capture includes readable Keyboard values and an explicit editor change opts one in; decoding and normalization preserve an older excluded flag, so upgrading cannot silently activate a formerly dormant saved value. Description, conditions, pointer speed, natural scrolling, standard-function-key behavior, display origin/rotation/active, mirroring, ColorSync, legacy network values, DNS/proxy, and system-output UID remain decodable and round-trip compatible, but the applicability normalizer excludes them from newly prepared apply payloads. The retired condition-editor source is removed from the app target while its Core schema and compatibility tests remain. Removing a control is therefore not destructive migration and cannot silently re-apply a hidden value.

`InputProfileSettings.keyboardBrightness` is an additive schema-1 field and decodes absent legacy values as excluded. The Core-owned profile stores only its scalar value. The system boundary discovers a compatible CoreHID backlight element at runtime, rejects zero or multiple matches and unreadable/unwritable elements as nonfatal availability results, captures the prior scalar for rollback, applies one operation, and requires immediate read-back agreement. Repeat speed and delay follow the same adapter lifecycle through experimental undocumented CFPreferences keys. Pointer speed, natural scrolling, and standard-function-key behavior remain dormant in both normal and available-items planning. No current Input path has live mutation or hardware-verification evidence.
...
`AudioProfileSettings.outputMuted` follows the same adapter boundary with a Boolean per-output-device catalog. The core model owns inclusion and desired state; the system adapter owns public Core Audio reads/writes, target-device planning, exact read-back, and rollback; SwiftUI owns only the localized toggle and unavailable-control repair row.
