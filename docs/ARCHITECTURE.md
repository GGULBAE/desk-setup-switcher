...
The store keeps a canonical document, a last-known-good backup, same-directory private staging files, and a quarantine directory. A replacement is fully encoded, written, and permissioned `0600` before one POSIX atomic rename; no fallible permission change occurs after the destination is replaced. Recovery decisions are reported to the UI. Every decoded, loaded, updated, or imported document passes the idempotent applicability normalizer before becoming authoritative, preserving snapshot values while excluding unsupported mutations. Temporary-directory tests cover valid reload, normalization, failed candidate update, staged permission/rename failure, corrupt primary recovery, canonical backup-first recovery, and corrupt primary-plus-backup reset. Sudden-power-loss durability is not claimed; public custom file-name validation and explicit no-follow protection for managed directory/file paths remain follow-up hardening.
