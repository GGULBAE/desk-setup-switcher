...
The profile row asks `PrimaryApplyActionState` for one action. A complete executable plan selects normal Apply; a partial plan with executable items selects available-items/force Apply in the same slot. Locks, pending protected-change confirmation, preparation, zero-operation, and unavailable states carry typed non-color reasons. Cached readiness may remain usable during a refresh, while per-profile preparation prevents duplicate requests.
...
The tray/settings end-to-end contract passed integrated non-live `make verify` on 2026-07-16: localization and source-policy lint; 366 default cases (134 XCTest plus 232 Swift Testing cases, five opt-in skips); Swift Debug/Release; universal Xcode Debug/Release; Analyze; package/checksum; mounted metadata/resources/`x86_64 arm64`; and ad-hoc/no-Developer-ID signature classification. `git diff --check` passed separately. The verified DMG SHA-256 is `76bc6d9f1187ea30f68be16ee81ee4a334d877a4e26c2497f35a9ffc781678b3`. Twelve tray and 10 current profile-editor detached PNG/AX pairs cover the tray tiers plus Display/ColorSync, supported/unsupported Audio, Ethernet/Wi‑Fi DHCP/manual, English/Korean, minimum width, accessibility large text, and dark mode; they do not prove installed tray/window ordering, native localization context, VoiceOver, or hardware behavior.
