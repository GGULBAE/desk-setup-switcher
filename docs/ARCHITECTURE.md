...
The current capture-permission tree passed integrated non-live `make verify` on 2026-07-14: lint/localization policy; 300 default cases (129 XCTest plus 171 Swift Testing cases, six opt-in skips), zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; package/checksum; mounted metadata/resources/`x86_64 arm64`; and ad-hoc/no-Developer-ID signature classification. Its separate five-case opt-in live-read gate passed on the current source, including the online-but-inactive display sleep state, without mutation. Separately, `git diff --check` passed. The verified DMG SHA-256 is `ed52b253159e6abc8fe35e606aed56cc269693a53b76986dae20a04ffb2bd4fc`; its matching binary was installed and launched from `/Applications`. The capture-permission implementation commit is `b34dfd7`; no push or new CI run was performed.
