...
The images were captured from the current Xcode `Debug` app bundle, not the SwiftPM command-line executable, so SwiftUI main-bundle localization is represented. Screenshots used the audit window ID. AX logs were produced by reading attributes, children, and available action names. No AX action, AppleScript, `AXPress`, UI automation, mouse event, or keyboard event was executed for these 12 synthetic captures. A later, separately recorded installed-app interaction used a user-authorized scoped exception and is not part of the synthetic evidence.
...
The required same-window 980→680→980 interaction still needs an explicit user result. Until that is supplied, preservation of the selected profile, expanded group/option, unsaved value, and focus across the live resize remains **pending**, even though the view identity is preserved in source. The actual MenuBarExtra Settings gear open/close/reopen path passed the later scoped installed-app interaction recorded in [13-installed-permission-handoff.txt](evidence/manual-ui-audit-2026-07-14/13-installed-permission-handoff.txt).

The later inline-delete-confirmation correction is source/build verified but is not depicted in these 12 static captures. Its confirmation is deliberately part of the profile card rather than a system dialog, avoiding the observed MenuBarExtra dismissal, and its list viewport now reserves additional height while confirmation is open to avoid clipping either action. The later capture-permission correction is also absent: current source explains and requests undetermined Location access, routes denied access to macOS System Settings or an explicit Wi-Fi-free path, and omits snapshot-only/unreadable/unsupported noise from the result card. Permission actions synchronously ask SwiftUI to open the app's persistent System tab before the delayed TCC or macOS System Settings action; a deterministic test fixes that ordering without invoking live system UI. The installed Capture purpose explanation and Continue-to-System-window handoff passed the later scoped interaction. The exact permission-result-card action was not produced after macOS resolved Location as already allowed, so inline deletion and the denied/granted TCC branches remain separate checks.

## Installed capture-permission handoff

On 2026-07-14 the user explicitly authorized a scoped automatic interaction with the installed package, DMG SHA-256 `df674b37e2a2fe9a94d37435313265069e27f6130ae05ad02b8ae822ac00e8b6`. The actual status item exposed Capture and Settings; Capture showed the Location purpose explanation; and Continue produced the persistent app System window at both 100 ms and 600 ms before any possible system-permission focus transfer. macOS resolved the generic Location state as already allowed, so no new grant/deny prompt or actionable permission result card appeared. TCC was not reset or changed. The actual Settings gear also passed open/close/reopen. The complete profile/diagnostic store was backed up and restored, temporary profiles and diagnostics were removed, and preflight/restored store hashes matched. The sanitized record is [13-installed-permission-handoff.txt](evidence/manual-ui-audit-2026-07-14/13-installed-permission-handoff.txt).

## Remaining capture-permission matrix

The remaining denied/granted matrix is intentionally user-driven. Do not use UI automation, AX actions, AppleScript, or an automated TCC reset for these future branches without a new explicit authorization. Record only pass/fail and visible generic state text; do not record or screenshot an actual SSID, IP address, location, or device identifier.
...
4. Repeat **Capture**, then select **Open macOS System Settings**. Confirm that the app's persistent **System** tab appears before macOS System Settings opens; the tray may close because macOS transfers focus, but it must not disappear without that visible handoff. Navigate manually to Privacy & Security > Location Services. The app deliberately uses the public application-open path rather than a private preference-pane URL.
...
6. On a clean account where authorization is genuinely **Not Determined**, select **Capture**, verify the app's purpose explanation appears, and choose **Continue**. Confirm that the persistent app **System** tab opens before the macOS prompt, then choose the intended macOS response. If this Mac is already determined, record this branch as not run instead of resetting TCC automatically.
...
The synthetic host is evidence for the contained SwiftUI content only. It does not prove the actual MenuBarExtra chrome, status-item anchor, or menu placement. The separate installed interaction proves the actual status item, Capture and Settings header actions, and Settings reactivation only; it does not broaden the synthetic layout evidence or prove profile-row actions.
