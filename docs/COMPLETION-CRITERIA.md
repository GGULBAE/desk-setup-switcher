...
| Current native UI structural audit | [Native UI structural reliability audit](UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md) records the installed symptom, first-proposal selective safe-area boundary, window cancellation/liveness audit, adaptive minimum workflow, focus policy, synthetic evidence, and explicit native/manual gap |
| Current focused UI regression gate | Final non-live verification passed 52 tests across five suites; an independent reviewer passed its 43-test structural subset. Native `T3/L11/B5/R2` reaches SwiftUI as `T3/L0/B5/R0` with content at `x16…352/y21…297`; same-turn close→reopen, coalesced-caller cancellation, minimum 520×360 layout, and safe initial focus policies pass |
| Current native UI structural full local gate | `make verify` passed on 2026-07-17: 412 default cases (144 XCTest with five opt-in skips + 268 Swift Testing across 35 suites with two default-disabled opt-in cases), zero failures; lint/localization policy; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted resources/architectures; and ad-hoc/no-Developer-ID classification |
...
| Most recent synthetic UI evidence | The [structural audit](UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md) adds one 680×480 Korean Settings fixture and two 520×360 Korean workflow fixtures under simulated accessibility text, plus opaque-RGB metadata and pixel checks. The preceding [UI stability audit](UI-STABILITY-AUDIT-2026-07-16.md) retains the broader tray/Settings/preview matrix. Actual popover chrome, native localization context, and complete VoiceOver behavior remain explicitly unproven |
...
| Previously installed app | The preceding UI-stability package replaced `/Applications/Desk Setup Switcher.app`; bundle ID, version 0.1.0, `x86_64 arm64`, and signature passed read-only checks. It was not launched after replacement. The current structural-fix package was not installed |
| Current follow-up package | Universal no-Developer-ID DMG SHA-256 `84bcdeac1f44de6381f93c8b9650132e26c4e9126c3d7137276db9c43274bb86`; checksum, mount, metadata/resources, architectures, and signature classification passed; it was not installed or launched |
...
- [x] The structural reliability follow-up filters native horizontal safe area before hosting, preserves vertical popover chrome, bounds and independently cancels persistent-window presentation, defends same-turn reopen, and adapts every workflow action/result surface plus keyboard/accessibility focus at the 520×360 minimum. Evidence is recorded in [UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md](UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md).
- [x] The structural reliability follow-up `make verify` passed 412 cases (144 XCTest with five skips + 268 Swift Testing across 35 suites with two disabled opt-in cases), all build/Analyze/package stages, and mounted universal-DMG verification. SHA-256 is `84bcdeac1f44de6381f93c8b9650132e26c4e9126c3d7137276db9c43274bb86`.
...
- [ ] The current build's installation plus actual first/repeated tray alignment, red-close→reopen destination behavior, native bilingual rendering, keyboard traversal, and VoiceOver initial focus are manually verified.
