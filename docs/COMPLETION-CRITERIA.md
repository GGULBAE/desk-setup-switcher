...
This ledger separates current implementation/test/local-commit gates from historical package/CI evidence and optional or unapproved manual hardware/release work. Tray Surface v2 is the current production source. User-provided installed screenshots exposed a native asymmetric empty-state inset and ambiguous Apply-to-review wording. The follow-up source is covered by non-live tests and was reinstalled, but it was not launched after replacement and has not completed the native interaction checklist. All `MenuBarExtra` evidence is historical. Hardware-dependent rows may have mock/read-back evidence while remaining explicitly “not hardware-verified.”
...
| Current UI-stability full local gate | `make verify` passed on 2026-07-16: 375 default cases (134 XCTest with five skips + 241 Swift Testing with two disabled opt-in cases), zero failures; lint/localization policy; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted resources/architectures; and ad-hoc signature classification. Separately, `git diff --check` passed |
...
| Current installed app | The verified follow-up package replaced `/Applications/Desk Setup Switcher.app`; bundle ID, version 0.1.0, `x86_64 arm64`, and signature passed read-only checks. It was not launched after replacement. No UI automation, TCC action, profile-store mutation, or system-setting mutation occurred |
| Current follow-up package | Universal no-Developer-ID DMG SHA-256 `516b968718aeb3c1c247e1a2deca7a28d45820d119f31ea992d4b475071f3638`; checksum, mount, metadata/resources, architectures, and signature classification passed |
...
| Historical install smoke | Earlier scoped installed interactions exercised the status item, deletion Esc/Cancel/Confirm, Settings resize/state preservation, Capture purpose explanation, and permission handoff. The current UI-stability package replaced the bundle but was deliberately not launched, so those earlier interactions do not prove the current source |
...
- [x] The UI-stability follow-up `make verify` passes 375 cases (134 XCTest with five skips + 241 Swift Testing with two disabled opt-in cases), all build/Analyze/package stages, and mounted universal-DMG verification. SHA-256 `516b968718aeb3c1c247e1a2deca7a28d45820d119f31ea992d4b475071f3638` is synchronized here.
...
- [x] The preceding canonical-plan implementation commit `8bd70dd` is recorded; the current UI-stability behavior-focused commit is reported in the final handoff. Nothing was pushed or published.
- [ ] The reinstalled build's actual empty tray alignment, Review→Apply Profile interaction, tray/window behavior, native bilingual rendering, keyboard traversal, and VoiceOver are manually verified.
