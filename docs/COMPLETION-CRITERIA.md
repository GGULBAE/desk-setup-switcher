...
| Current tray/editor full local gate | `make verify` passed on 2026-07-14: lint/localization policy; 298 default cases (129 XCTest plus 169 Swift Testing cases, six opt-in skips), zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted resources/architectures; and ad-hoc signature classification |
| Current synthetic UI evidence | [Manual UI audit](MANUAL-UI-AUDIT-2026-07-14.md) records 12 final-source English/Korean PNGs and 12 read-only AX logs. Static overview, standard editor, 680×480 minimum, simulated large-text, validation, System, and advanced-diagnostics states passed visual/AX/privacy review. Overview evidence covers deletion and partial-capture explanations; editor evidence covers the wider card layout. No UI automation or system mutation was used |
...
| Current verified local package | Tray/editor universal no-Developer-ID DMG with ad-hoc-signed app; SHA-256 `273155843df1260ffb7555fe447d895e95696c95c28247fb023ad8f8ee2d39eb`. It passed checksum, mount, metadata, resources, architectures, and signature classification. Preceding layout-correction SHA-256 `ae940ce1cffb969f309b8ffa8f6ffcd0637fd0845ee21bf24fa59129ea530ef7` and apply-reliability SHA-256 `417ffbb20b6a77b9037f42d5acb998574460374675e746715474e17f9f772615` are historical |
...
- [x] The final-source tray/editor tree passed 298-case `make verify`, replacement package/checksum verification, and the separate opt-in five-case live-read smoke run on 2026-07-14 without mutation.
...
- [x] Partial-capture details retain sanitized item names and per-category reasons for explanation while normalizing runtime display/audio/network identifiers out of presentation state.
...
- [x] Menu/apply source uses one state-aware primary slot plus direct Edit and a named destructive Delete control. Delete confirmation stays inside the profile card so it cannot dismiss the MenuBarExtra; Cancel/`Esc` changes no storage. Ready + operations selects normal Apply and Partial + executable operations selects Apply Available. Ellipsis, separate availability review, and manual refresh remain absent; deterministic synthetic cases cover zero-operation, cached refresh, preparing, lock, and confirmation states.
...
- [x] Current-source English/Korean standard editor, overview, validation, System, advanced diagnostics, 680×480 minimum, and simulated large-text states passed static visual/AX review with synthetic data. The refreshed overview verifies profile deletion controls and itemized partial-capture explanations; editor states verify expanded Audio/default-device cards and Include-switch alignment. The fixed responsive breakpoint intentionally replaces the former draggable split divider.
...
- [x] The tray/editor follow-up produced and verified the current replacement universal package/checksum: SHA-256 `273155843df1260ffb7555fe447d895e95696c95c28247fb023ad8f8ee2d39eb`.
