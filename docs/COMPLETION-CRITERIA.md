...
| Current UI-audit full local gate | `make verify` passed on 2026-07-14: lint/localization policy; 297 default cases (129 XCTest plus 168 Swift Testing cases, six opt-in skips), zero failures; Swift Debug/Release; universal Xcode Debug/Release; Analyze; DMG/checksum; mounted resources/architectures; and ad-hoc signature classification |
| Current synthetic UI evidence | [Manual UI audit](MANUAL-UI-AUDIT-2026-07-14.md) records 12 final-source English/Korean PNGs and 12 read-only AX logs. Static standard, 680×480 minimum, simulated large-text, validation, permission, and diagnostics states passed visual/AX/privacy review. Refreshed standard editor evidence covers the corrected Audio/default-device title and compact Include-switch alignment. No UI automation or system mutation was used |
...
| Opt-in live reads | On 2026-07-14, display, audio, network, input, and combined readiness-context tests all passed with `DESK_SETUP_LIVE_READ_TESTS=1`; the live Keychain write remained skipped and no setting mutation was run |
...
| Current verified local package | UI-audit/layout-correction universal no-Developer-ID DMG with ad-hoc-signed app; SHA-256 `ae940ce1cffb969f309b8ffa8f6ffcd0637fd0845ee21bf24fa59129ea530ef7`. It passed checksum, mount, metadata, resources, architectures, and signature classification. Pre-layout-fix UI-audit SHA-256 `fa42df665ca453165dd041fe5112a8f1a6d2314bd31512ef9371b12289132122` and apply-reliability SHA-256 `417ffbb20b6a77b9037f42d5acb998574460374675e746715474e17f9f772615` are historical |
...
- [x] The final-source UI-audit tree passed 297-case `make verify`, replacement package/checksum verification, and the separate opt-in five-case live-read smoke run on 2026-07-14 without mutation.
...
- [x] Menu capture returns a value-free complete/partial/failure summary, counts applicable/snapshot-only/unreadable/permission/unsupported outcomes, rejects captures with no applicable leaf, and does not expose an SSID or request permission automatically.
...
- [x] Setting inclusion and disclosure are independent in source; collapsing preserves configured values. Category/option titles keep width priority beside compact fixed-width Include controls, avoiding the observed Korean Audio-row compression. Common display/audio/network/input values use typed pickers, toggles, semantic sliders, and a typed ephemeral display-mode catalog, while unsupported snapshot-only values are read-only and never persisted as catalog metadata.
...
- [x] Menu/apply source uses one state-aware primary slot plus direct Edit: Ready + operations selects normal Apply and Partial + executable operations selects Apply Available. Ellipsis, separate availability review, and manual refresh remain absent; deterministic synthetic cases cover zero-operation, cached refresh, preparing, lock, and confirmation states.
...
- [x] Current-source English/Korean standard editor, overview, validation, permission, diagnostics, 680×480 minimum, and simulated large-text states passed static visual/AX review with synthetic data. The refreshed standard editor states explicitly verify expanded Audio/default-device title and Include-switch alignment. The fixed responsive breakpoint intentionally replaces the former draggable split divider.
...
- [x] The UI-audit/layout-correction follow-up produced and verified the current replacement universal package/checksum: SHA-256 `ae940ce1cffb969f309b8ffa8f6ffcd0637fd0845ee21bf24fa59129ea530ef7`.
