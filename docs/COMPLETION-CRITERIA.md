...
| Current UI/UX installed audit | [UI/UX simplification and installed-app audit](UI-UX-SIMPLIFICATION-INSTALLED-AUDIT-2026-07-17.md) records the final attached-wrapper root cause, native popover regression, persistent-window clamps, async-generation ownership, installed screenshots/geometry, package and unchanged safety-state evidence, and explicit manual-accessibility gap |
...
| Most recent installed/synthetic UI evidence | The [installed evidence index](evidence/ui-ux-installed-2026-07-17/README.md) records final installed tray/Settings/workflow screenshots, geometry, package hashes, unchanged store/defaults invariants, and synthetic large-text/error fixtures. The [structural audit](UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md) and [UI stability audit](UI-STABILITY-AUDIT-2026-07-16.md) remain preceding matrices. Complete VoiceOver and focused-control AX behavior remain explicitly unproven |
| Installed interaction evidence | The current package was reinstalled to `/Applications`. Twenty measured popover opens had exact center delta `0`; Settings clamped `500×300` to `680×480` and retained `900×568` across ten normal opens; workflow clamped `500×300` to `520×392`, opened at frame `620×532` with `620×500` content, and closed with Escape. Apply, Capture, TCC, login, and system-setting mutations were not invoked |
...
| Current follow-up package | Universal no-Developer-ID DMG SHA-256 `82b05e6a3b1b20978c85c4d82d0976237a731c76c07bcd7d1729a84e3e0b6f21`; checksum, mount, metadata/resources, `x86_64 arm64`, and signature classification passed. The reinstalled `/Applications` executable SHA-256 is `40876bd671dd4286fa4684192097f6dbd702df899bccf8efb588b244c3d27305` |
...
## Current bounded P2 UI follow-up ledger

- [ ] Empty-tray Capture has an adequately prominent call to action.
- [ ] Duplicate, Reorder, Import, and Export have a clear hierarchy, and Export semantics are explicit when the selected profile has a dirty draft.
- [ ] Included toggles communicate their meaning without ambiguity.
- [ ] `DisclosureGroup` is manually verified with VoiceOver; synthetic focus intent is not accepted as completion.
- [ ] Advanced Diagnostics' 700-point minimum is reconciled with the Settings window's 680-point minimum.
- [ ] About, protected-change safety, and result states have minimum-size and accessibility fixtures.
- [ ] `⌘,` reliably forces the intended Settings tab.
- [ ] Generic storage errors have sufficient prominence.

This P2 ledger is the complete next-task scope. Full VoiceOver use and focused-control AX observation remain manual evidence; profile-store path hardening, Apply/Capture execution, TCC/login changes, hardware mutation, and release work are not folded into this pass.
