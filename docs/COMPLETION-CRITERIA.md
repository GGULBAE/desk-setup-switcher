...
- [x] The final non-live `make verify` passes localization/source policy, 338 cases (132 XCTest + 206 Swift Testing, six opt-in skips), Swift and universal Xcode Debug/Release, Analyze, DMG/checksum, mounted metadata/resources/`x86_64 arm64`, and ad-hoc signature classification.
- [x] The verified local no-Developer-ID DMG SHA-256 is `539c203607782302799d68acdda2f64666f0ace5897fa325a79e1dfdcfc98f78`; it was not installed, launched, pushed, or published.
- [x] Twenty deterministic reopen cycles retain fixed content size, zero AppKit host origins, and a fresh top-scroll request without height feedback or magic offsets.
- [x] Settings and Profile Edit each survive ten red-close/reopen cycles through the same app-lifetime controller/root, and destination visible/key/front completion precedes tray dismissal.
