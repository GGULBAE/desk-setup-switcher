...
This ledger separates current implementation/test/local-commit gates from historical package/CI evidence and optional or unapproved manual hardware/release work. Tray Surface v2 is the current production source. User-provided installed screenshots exposed a native asymmetric empty-state inset and ambiguous Apply-to-review wording. A preceding package was reinstalled without launch; the current settings-lifecycle/UI-declutter package is covered by the full non-live gate but was not installed or launched and has not completed the native interaction checklist. All `MenuBarExtra` evidence is historical. Hardware-dependent rows may have mock/read-back evidence while remaining explicitly “not hardware-verified.”
...
| Current canonical apply-plan full local gate | Historical immediately preceding gate: 371 default cases (134 XCTest with five skips + 237 Swift Testing with two disabled opt-in cases), zero failures, with the same build/Analyze/package stages passing |
| Most recent synthetic UI evidence | [UI stability audit](UI-STABILITY-AUDIT-2026-07-16.md) records 13 English/Korean-path attached/offscreen tray pairs, 10 full Settings-root pairs, and three workflow pairs. Empty/1/3/10 profiles, overflow, deletion, permission, result, minimum Settings, dark, simulated large text, refreshed Korean, and 520×360 force/large-text preview states passed visual/size/privacy review. Forced-locale hosts leave some system-localized controls in English; actual popover chrome, native localization context, and complete VoiceOver behavior remain explicitly unproven |
...
| Previously installed app | The preceding UI-stability package replaced `/Applications/Desk Setup Switcher.app`; bundle ID, version 0.1.0, `x86_64 arm64`, and signature passed read-only checks. It was not launched after replacement. The current refactor package was not installed |
| Current follow-up package | Universal no-Developer-ID DMG SHA-256 `f3aa610026179161208dec2cb2ef6185768843becd8e0e56bccc9f8abab37f2b`; checksum, mount, metadata/resources, architectures, and signature classification passed; it was not installed or launched |
...
- [x] The SwiftUI root ignores actual native horizontal safe-area values, and an injected asymmetric-safe-area raster regression distinguishes the correction from the prior shifted layout. The empty state uses an explicit compact stack rather than native minimum-height unavailable content.
...
- [x] The preceding canonical-plan implementation commit `8bd70dd` and UI-stability commit `8a1e80e` are recorded; the current refactor commit is reported in the final handoff. Nothing was pushed or published.
- [ ] The current build's installation plus actual empty tray alignment, Review→Apply Profile interaction, tray/window behavior, native bilingual rendering, keyboard traversal, and VoiceOver are manually verified.
