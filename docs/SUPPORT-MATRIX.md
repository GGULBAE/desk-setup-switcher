...
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0; the current UI tree passes full local `make verify` with 215 default non-live tests on macOS 26.5.2 with Xcode 26.6. The 158-test 2026-07-11 baseline remains recorded below for historical live-read/install evidence |
...
| Distribution | Direct no-Developer-ID DMG | Current local header/editor artifact, SHA-256 `45772d20e6d7655c41ed4ff5d0261257b98f1361f4cf8cc38ebf837720d5820b`, passed checksum, mount, metadata/resources, architectures, and ad-hoc app-signature verification. Downloaded CI artifact ID `8256718472`, SHA-256 `f3d82b033e8e375c9063a9b72cbd174d94a03f0cdd4414961895db3b3dcfc3f4`, predates the follow-up; not published and Gatekeeper path pending |
...
| Profiles | Saved/draft editing, replacement protection, and metadata merge | Unit verified with 29 deterministic draft tests; source/build reviewed for fixed save/revert controls, `⌘S`, app-lifetime ownership, ordinary-quit protection, and async storage result handling. No current-tree manual draft walkthrough |
...
| Profiles | Expandable direct setting-value editor | Source/build verified for group and per-option expansion with concrete display/audio/network/input controls and explicit unsupported notices. Deterministic draft coverage verifies disabling and re-enabling preserves values and legacy conditions. No current-tree layout walkthrough |
...
| Readiness | Legacy condition persistence and evaluation | Unit/mock verified. Condition editing is no longer exposed in Settings; existing and imported conditions remain preserved and evaluated. Earlier choice/input-validation tests remain regression coverage, not current-UI evidence |
...
| Apply | Menu action state and friendly preview presentation | Unit verified for Ready/Partial/Unavailable/Applying, transaction-lock, zero-operation, rejection-reason, technical-detail, and internal review/normal/available-items presentation states. Source/build verified for Apply, direct Edit, compact Capture, icon-only Settings/Quit, typed capture failure, brief success-only feedback, automatic menu-open readiness refresh, and removal of ellipsis/availability-review/available-items/manual-refresh controls; no current-tree manual click-through or execution |
