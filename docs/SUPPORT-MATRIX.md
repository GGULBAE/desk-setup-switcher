...
“Unit verified,” “mock verified,” and “live-read verified” must never be shortened to “hardware verified.” As of 2026-07-17, **no live setting mutation is hardware verified**. The current tray/Settings/workflow contract is deterministic mock/attached-offscreen verified. Its package is build/mount verified but was not installed or launched; a preceding installed package and its screenshots do not prove the current first attach, window focus, ColorSync, audio, or IPv4 behavior.

## Tray Surface v2 evidence

| Contract | Automated/offscreen evidence | Manual status |
| --- | --- | --- |
| One status item, popover, and hosting controller | Factory/count/behavior tests verify one `.applicationDefined` popover, one existing `NSHostingController`, a public parent content controller, matching zero-origin host bounds, and stable ownership across reopen | Actual status-item click, anchor, arrow, material, and ghost-frame behavior pending |
| Empty-state horizontal layout | An attached AppKit boundary converts native `T3/L11/B5/R2` to hosted `T3/L0/B5/R0`, preserving content at `x16…352/y21…297`. A zero/filtered/unfiltered three-way raster proves the filtered asymmetric result matches baseline and the unfiltered control moves | Reinstalled native first-open and repeated-open tray alignment remains user-confirmation pending |
...
| Action disposition and handoff | Every action is mapped; stay-open cannot close; visible/key order, bounded non-key liveness, independently cancellable coalesced callers, red-close, same-turn immediate reopen, stale completion, failure, and quit-only termination pass | Native focus-transfer timing and first responder pending |
| App-lifetime state/workflows | Dismiss/reopen task survival, deletion focus, persistent-window ownership, synthetic permission suppression, minimum-size adaptive actions, safe keyboard focus policy, and accessibility focus intent pass | Full keyboard and VoiceOver speech/rotor walkthrough pending |
| Catalog parity and bilingual-path rendering | The preceding 13 tray, 10 Settings-root, and three workflow PNG/metadata pairs pass size/privacy checks. The [current structural audit](UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md) adds 680×480 Korean Settings and two 520×360 Korean workflow states with simulated accessibility text and opaque RGB/pixel checks. Some system-localized controls remain English in the forced-locale host | Native localization context, complete linguistic review, real text size/contrast/transparency, single-frame flicker, and complete accessibility tree pending |

## Platform

| Area | Target | Current evidence |
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0. The current structural-fix source passed full local `make verify` on 2026-07-17 with 412 default cases (144 XCTest plus 268 Swift Testing cases across 35 suites), five skipped and two disabled opt-in cases, zero failures, and no live flag. Earlier gates remain historical evidence |
...
| --- | --- | --- |
| App | Background/menu-bar-only lifecycle | `LSUIElement` and the app-owned `NSStatusItem`/`NSPopover` source are build/package verified. Persistent Settings/workflow presentation now has a liveness deadline, consumer-local cancellation, red-close detachment, immediate-reopen defense, and identity-guarded stale completion. The current DMG was not installed or launched |
...
| Stable Settings workspace | Policy and owned-window geometry tests cover one 210-point sidebar + 390-point-minimum editor anatomy, resizable style, 680×480 minimum, fixed footer/action bars, at-most-one refresh adoption per presentation, no reset when invoking an already-visible window, and stable window ownership | Attached/offscreen full-root 900×568 and 680×480 evidence passes. Historical installed 980→680→980 preserved selected row, valid dirty draft, focus, Save availability, and expanded Audio state; current installed resize/flicker remains manual |
...
- A current installed native status-item/Settings/workflow walkthrough with real text size and full keyboard/VoiceOver/focus/contrast/transparency review

## Tray/settings end-to-end and structural evidence recorded through 2026-07-17

The current native UI structural non-live gate passes 412 default cases (144 XCTest with five opt-in skips plus 268 Swift Testing across 35 suites with two default-disabled opt-in cases), Swift/Xcode Debug and Release, Analyze, and universal mounted-package verification. Current local DMG SHA-256 is `84bcdeac1f44de6381f93c8b9650132e26c4e9126c3d7137276db9c43274bb86`; it was not installed or launched. The settings-lifecycle and UI-stability gates plus the installed package remain historical evidence.

| Capability | Implementation boundary | Current evidence |
| --- | --- | --- |
| Tray reopen geometry and anchor | 368-point width; 260/260/316/560-point 0/1/2/3+ tiers; static non-scrolling empty body; symmetric 16-point content inset; generation-keyed top scroll; one internal overflow scroll view; status-item width frozen while open | Deterministic tests use a title-sized anchor spy, asymmetric native safe area, deliberately displaced geometry, stale completion rejection, two-profile bottom gap, late first layout, and 20 reopen generations without sleeps. Actual menu-bar anchor, popover material, dismissal, and single-frame installed flicker remain manual |
| Apply-preview/workflow layout | Persistent 620×500 workflow window with 520×360 minimum, child screens fill one canvas, request-ID top reset, scrollable body above adaptive measured actions, responsive grid/stack rows, scrollable protected summaries, and monospaced combined countdown | Attached/offscreen initial/refreshed previews plus Korean permission and dirty-Apply minimum/accessibility-text states pass. Native transition timing, full keyboard/VoiceOver order, and any hardware-changing confirmation remain manual |
| Persistent Settings/Profile Edit | App-lifetime controllers; red-close detaches then cancels the current producer; visible/key/front completion precedes tray close; shared `.regular`-while-visible/`.accessory`-when-hidden coordinator | Deterministic tests cover 10 reopen cycles, two-window activation, non-key deadline, consumer-local cancellation, coalescing, same-turn immediate reopen, stale completion, and unsaved-root identity. Native app/Space switching and installed window ordering remain manual |
