...
“Unit verified,” “mock verified,” and “live-read verified” must never be shortened to “hardware verified.” As of 2026-07-18, **no live setting mutation is hardware verified**. The P2 tray/Settings/workflow source passed the 461-check gate and its 39-fixture/79-artifact attached/offscreen review. The final DMG was reinstalled for bounded non-mutating checks; neither P2 nor the preceding geometry pass invoked Apply, Capture, TCC, login, or system-setting mutation. Installed disclosure Return/Space and AX state updates passed. VoiceOver was not run, was explicitly removed from P2 completion scope, was restored disabled, and is not claimed. The bounded check does not prove full keyboard traversal, full-app VoiceOver/rotor, complete focused-control AX state, ColorSync, audio, or IPv4 behavior.
...
| Empty-state horizontal layout and CTA | The structural boundary still converts native `T3/L11/B5/R2` to hosted `T3/L0/B5/R0`, preserving content at `x16…352/y21…297`. P2 placement policy adds one labelled body-level Capture CTA only for pristine empty/idle state and keeps one compact header action everywhere else | Preceding 20-open measurements reported exact center delta `0`; the final DMG reinstall and bounded changed-surface inspection passed. Capture itself was not invoked |
...
| App-lifetime state/workflows | Dismiss/reopen task survival, deletion focus, persistent-window ownership, synthetic permission suppression, minimum-size adaptive actions, safe keyboard focus policy, and accessibility focus intent pass. App-owned disclosures expose explicit localized state/hint and remove collapsed children | Installed Return expands and Space collapses with AX value/hint/child presence updated. VoiceOver was not run and is a nonblocking unverified follow-up; no VoiceOver claim is made |
| Catalog parity and bilingual-path rendering | The [P2 audit](P2-UI-REFINEMENT-AUDIT-2026-07-17.md) and [P2 evidence index](evidence/p2-ui-refinement-2026-07-17/README.md) record 39 fixtures and 79 generated and inspected artifacts: 15 tray, 13 Settings, three apply-preview, two responsive-workflow, and six auxiliary fixtures, plus one Settings footer comparison note | Native linguistic review, real text size/contrast/transparency, full-app VoiceOver, and complete focused-control AX observation remain pending and nonblocking for P2 |

## Platform

| Area | Target | Current evidence |
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0. P2 final integrated result is 461 checks (144 XCTest + 316 Swift Testing across 39 suites + one isolated native popover regression); the preceding structural gate passed 446 checks |
...
| Distribution | Direct no-Developer-ID DMG | P2 DMG SHA-256 is `342d804d8bbff51209af4bccefb405ee76499050c1e640a011d41e2f78792031`; reinstalled ad-hoc-signed executable SHA-256 is `fb35352fb6a9588c0c50269975ccd3d7b73e52010de10de132bf45d60236f719`. The app launched with `x86_64 arm64`; profile primary/backup/defaults remained unchanged. The preceding structural hashes are historical. Nothing is published and the quarantined Gatekeeper path remains pending |
...
| Profiles | Atomic primary/backup, corruption quarantine/recovery, local file permissions | Temporary-directory tests verify 0700 directories, 0600 managed/staged files, last-known-good recovery, pre-commit permission failure, and no post-replacement permission step. Sudden-power-loss durability is not claimed |
| Profiles | Generic storage-error recovery | Generic failures use a prominent error heading/icon/card instead of only a footer caption, disable the profile workspace while global recovery owns the surface, and clear stale editor feedback. Initial load offers one Retry Loading action; ordinary operation failure offers one Dismiss Error action; editor-owned save failure suppresses a duplicate global error. Temporary-store tests cover workspace/error ownership, action ownership, and repeated failed retry. This does not add profile-store path hardening |
| Profiles | Import/export | Existing size/schema/semantic, duplicate-ID, source-protection, and no-overwrite behavior remains mock verified. P2 names the action **Export Saved Profiles…**, shows a dirty-draft saved-only notice, and tests that export reads the persisted document without merging the editor draft. Interactive file-panel/import/export review remains pending |
...
| Keyboard and accessibility structure | Localized AX policies cover labels, values, descriptions, validation metadata, de-duplicated profile rows, and explicit disclosure state/hint | Installed Return expands and Space collapses with AX value/hint/child presence updated. VoiceOver was not run, was explicitly removed from P2 completion scope, and is not claimed. Full keyboard-only order and VoiceOver behavior remain nonblocking follow-ups |
...
- Full installed keyboard/VoiceOver/focused-control AX, real text size, contrast, and transparency review

## Current P2 and preceding structural evidence recorded through 2026-07-17

The [P2 UI refinement audit](P2-UI-REFINEMENT-AUDIT-2026-07-17.md) is the current UI evidence boundary. Its final integrated count is 461, DMG SHA-256 is `342d804d8bbff51209af4bccefb405ee76499050c1e640a011d41e2f78792031`, and the reinstalled ad-hoc-signed executable SHA-256 is `fb35352fb6a9588c0c50269975ccd3d7b73e52010de10de132bf45d60236f719`. The app launched with `x86_64 arm64`; profile primary/backup/defaults remained unchanged. The final DMG reinstall, `⌘,` routing, and installed disclosure keyboard/AX checks passed without Apply/Capture/TCC/login/hardware mutation. VoiceOver process and preference were both 0; VoiceOver was not run and is not claimed. The preceding audit retains the 446-check structural gate and historical package/install hashes.
...
This bounded pass must not be reported as a completed full-app VoiceOver, rotor, keyboard-order, or focused-control AX walkthrough. It does not include profile-store path hardening, Apply/Capture execution, TCC/login changes, Gatekeeper, physical Intel, diagnostics clearing, or hardware mutation.
