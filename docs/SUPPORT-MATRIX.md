...
| Catalog parity and bilingual-path rendering | 13 attached/offscreen tray, 10 full Settings-root, and three workflow PNG/metadata pairs pass size/privacy checks; English and forced-Korean paths, light/dark, 680×480 Settings minimum, 520×360 workflow minimum, large text, ColorSync, supported/unsupported audio, Ethernet/Wi‑Fi DHCP/manual, and initial/refreshed previews are represented. Some system-localized controls remain English in the forced-locale host | Native localization context, complete linguistic review, real text size/contrast/transparency, single-frame flicker, and complete accessibility tree pending |

## Platform

| Area | Target | Current evidence |
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0. The UI-stability source passed full local `make verify` on 2026-07-16 with 375 default cases (134 XCTest plus 241 Swift Testing cases), five skipped and two disabled opt-in cases, zero failures, and no live flag. Earlier gates remain historical evidence |
...
| Distribution | Direct no-Developer-ID DMG | Current `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg`, SHA-256 `f3aa610026179161208dec2cb2ef6185768843becd8e0e56bccc9f8abab37f2b`, passed checksum, mount, metadata/resources, `x86_64 arm64`, and ad-hoc/no-Developer-ID signature verification. It was not installed or launched. Earlier checksums remain historical; nothing is published and the Gatekeeper path remains pending |
...
| --- | --- | --- |
| App | Background/menu-bar-only lifecycle | `LSUIElement` and the app-owned `NSStatusItem`/`NSPopover` source are build/package verified. The current DMG was not installed or launched. Historical installed interactions verify only the behavior they directly exercised |
...
| Profiles | Flat typed setting-value editor | Display/Audio/Network are always expanded; each leaf owns Include and there is no group/option disclosure state. `VisibleSettingRegistry` projects only nine complete setting kinds from typed runtime catalogs; unsupported, read-only, missing, or ambiguous controls are absent as new choices. A persisted included Audio, ColorSync, or service-IPv4 target that becomes unavailable keeps a warning plus Include-off repair control. Pure validation covers primary uniqueness, modes, portable ColorSync identity, audio values, and per-service IPv4 fields. Current standard/minimum/large-text/dark/Korean evidence passed; interactive keyboard/VoiceOver and state-preserving resize remain pending |
...
The current settings-lifecycle/UI-declutter non-live gate passes 401 default cases (144 XCTest with five opt-in skips plus 257 Swift Testing with two default-disabled opt-in cases), Swift/Xcode Debug and Release, Analyze, and universal mounted-package verification. Current local DMG SHA-256 is `f3aa610026179161208dec2cb2ef6185768843becd8e0e56bccc9f8abab37f2b`; it was not installed or launched. The preceding UI-stability gate and installed package remain historical evidence.
...
| Apply-preview/workflow layout | Persistent 620×500 workflow window, child screens fill one canvas, request-ID top reset, fixed action footer, size-based bounce, scrollable protected summaries, monospaced countdown | Attached/offscreen initial English and refreshed Korean 620×500 previews pass. Native window transition timing, full keyboard/VoiceOver order, and any hardware-changing confirmation remain manual |
