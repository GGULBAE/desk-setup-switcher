...
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0. The stable permission-handoff tree passed full local `make verify` on 2026-07-14 with 301 default cases (129 XCTest plus 172 Swift Testing cases, six opt-in skips), zero failures, and no live flag. The 300-case capture-permission, 298-case tray/editor, 297-case layout-correction, 290-case apply-reliability, 215-test header/editor, and 158-test 2026-07-11 gates remain historical evidence |
| Apple Silicon | arm64 | Default tests passed on the current stable handoff source. The opt-in live-read smoke tests passed on the immediately preceding capture source on an Apple M5 Mac; the display gate covers both active displays and the legitimate online-but-inactive sleep state |
...
| Tray deletion | Pure state/height tests cover exact-ID confirmation, Cancel, and confirmation-region sizing; current English/Korean screenshot/AX pairs show complete actions | Installed Esc, Cancel, and Confirm all kept the tray available; one confirmed deletion was rolled back from backup. Full keyboard traversal remains pending |
