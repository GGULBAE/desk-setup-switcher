...
| Apply | Menu action state and friendly preview presentation | Unit verified for Ready/Partial/Unavailable/Applying, transaction-lock, zero-operation, rejection-reason, technical-detail, and review/normal/available-items presentation states. Source/build verified for primary Apply, direct profile editing into the Profiles tab, secondary available-items apply, automatic menu-open readiness refresh, and removal of menu availability-review/manual-refresh controls; no current-tree manual click-through or execution |
