...
“Unit verified,” “mock verified,” and “live-read verified” must never be shortened to “hardware verified.” As of 2026-07-14, **no live setting mutation is hardware verified**. The apply-reliability source and deterministic tests described below passed the full non-live local gate; its package is build/mount verified, not installed or hardware-mutation verified.

## Platform

| Area | Target | Current evidence |
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0. The apply-reliability tree passed full local `make verify` on 2026-07-14 with 290 default cases (124 XCTest with five skips plus 166 Swift Testing with one skip), zero failures, and no live flag. The 215-test header/editor and 158-test 2026-07-11 baselines remain historical evidence |
...
| Distribution | Direct no-Developer-ID DMG | Current `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg`, SHA-256 `417ffbb20b6a77b9037f42d5acb998574460374675e746715474e17f9f772615`, passed checksum, mount, metadata/resources, `x86_64 arm64`, and ad-hoc/no-Developer-ID signature verification. The preceding header/editor SHA-256 `45772d20e6d7655c41ed4ff5d0261257b98f1361f4cf8cc38ebf837720d5820b` and downloaded CI artifact ID `8256718472`, SHA-256 `f3d82b033e8e375c9063a9b72cbd174d94a03f0cdd4414961895db3b3dcfc3f4`, remain historical; nothing is published and the Gatekeeper path remains pending |
...
| App | English/Korean UI | Integrated lint passed key parity, duplicate-key, placeholder, and statically discoverable-key checks for state-aware Apply, capture/results, validation, and read-back states. The 2026-07-11 DMG rendered in Korean, but a complete rendered current-tree bilingual walkthrough remains pending |
| App | Accessibility | Current source contains icon labels/help, non-color state and validation text, inclusion/target values, save/apply/result announcements, display-confirmation default/Escape shortcuts, and a remaining-time accessibility value. One baseline label passed inspection on 2026-07-11; full current-tree VoiceOver/keyboard/focus/contrast/text-size behavior is not verified |
...
| Profiles | Saved/draft editing, replacement protection, and metadata merge | The preceding 29 deterministic draft tests unit-verify selection/replacement behavior. The follow-up's pure dirty-Apply and app-integration cases passed in the 290-case full gate: save/discard/cancel resolves before planning, save failure preserves the draft, and only the latest stored profile advances. No current-tree manual draft walkthrough |
...
| Snapshot | Detected/storable/unreadable/permission-required/unsupported item classification | Existing classification is mock verified and concrete adapters were live-read verified on one Apple Silicon Mac. The follow-up adds a value-free complete/partial/failure summary with saved/snapshot-only/unreadable/permission/unsupported counts; deterministic presentation cases are present |
...
This evidence is unit/mock/read-back only. It does not include a live Apply, actual display/audio/network/input mutation, hardware rollback, a new live read, UI automation, full VoiceOver, TCC grant/deny, Keychain write, physical Intel execution, Developer ID signing, notarization, or release publication.

## Source/test-backed synthetic UI review on 2026-07-14

No UI automation or system action was used. This review maps the requested states to pure tests and static SwiftUI/accessibility source; it is not a rendered screenshot, minimum-size layout, keyboard traversal, or VoiceOver speech/focus run.
...
| Unavailable and no-op | `ApplyWorkflowPresentationTests` verifies distinct disabled reasons | No rendered row-width review |
...
| Partial capture with permission and snapshot-only items | `ProfileCaptureSummaryBuilderTests` verifies sanitized counts and no raw Wi-Fi value | Capture was not invoked |
| Inline validation | `ProfileDraftValidationTests` verifies field identifiers, required/range/format errors, primary uniqueness, and Wi-Fi byte limits; source review confirms invalid metadata and first-error focus routing | No rendered focus traversal |
...
| Collapsed/expanded groups and options | `DirtyApplyProtectionTests` verifies `DisclosureState` never changes inclusion or values; source review confirms separate disclosure state for all groups | No long-content layout walkthrough |
| English/Korean and long strings | Catalog lint verifies parity, duplicates, placeholders, and statically discoverable keys | No rendered bilingual/minimum-size or linguistic-quality audit |
| Keyboard and accessibility structure | Source review confirms localized icon labels/help, include/target separation, invalid descriptions, announcements, default/Return and Escape contracts | Full keyboard-only and VoiceOver behavior remain pending |
