...
| Actionable capture permission result | Current `ProfileCaptureSummaryBuilderTests` prove snapshot-only, unreadable, unsupported, and runtime-identifier evidence is omitted while a permission-hidden Wi-Fi name remains value-free and actionable. Injected app tests cover authorization-request and System Settings-open routing without live TCC actions | The earlier overview PNG renders the retired category-count UI; current user-driven TCC click-through is pending |
