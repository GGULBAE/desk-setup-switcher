...
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0. The measured-height tray/responsive tree passed full local `make verify` on 2026-07-15 with 305 default cases (129 XCTest plus 176 Swift Testing cases, six opt-in skips), zero failures, and no live flag. The 301-case permission-handoff, 300-case capture-permission, 298-case tray/editor, 297-case layout-correction, 290-case apply-reliability, 215-test header/editor, and 158-test 2026-07-11 gates remain historical evidence |
| Apple Silicon | arm64 | Default tests passed on the current measured-height source. The opt-in live-read smoke tests passed on the preceding capture source on an Apple M5 Mac; the display gate covers both active displays and the legitimate online-but-inactive sleep state |
...
| --- | --- | --- |
| App | Background/menu-bar-only lifecycle | The current verified DMG completely replaced `/Applications/Desk Setup Switcher.app`; the installed binary matched the mounted source and launched successfully. `LSUIElement` is verified in the current bundle. A user-authorized scoped installed interaction verified the actual status item and Settings open/close/reopen without changing system settings, then restored the complete profile/diagnostic store |
...
| Profiles | CRUD, ordering, selection, metadata, per-group/per-option inclusion | Mock verified at the storage/domain boundary. Each menu profile has a named Delete control whose confirmation expands inside the profile card; the list viewport follows measured card content up to a scrolling cap rather than estimating from profile count. Cancel/`Esc` leaves storage untouched, while confirming a dirty selected profile explicitly discards its unsaved draft before calling the existing store path. A historical fresh install created one schema-v1 Ready profile from a read-only snapshot with all four groups; the measured-height build awaits a fresh installed visual check |
...
| Tray deletion | Pure state/height tests cover exact-ID confirmation, Cancel, measured-content sizing, and normal/confirmation scroll caps; current English/Korean screenshot/AX pairs show complete actions | Installed Esc, Cancel, and Confirm all kept the tray available; one confirmed deletion was rolled back from backup. The measured-height correction is deterministic-test verified but awaits a fresh installed visual check. Full keyboard traversal remains pending |
