...
“Unit verified,” “mock verified,” and “live-read verified” must never be shortened to “hardware verified.” As of 2026-07-17, **no live setting mutation is hardware verified**. The current tray/Settings/workflow contract is deterministic mock/attached-offscreen verified, and its bounded geometry behavior is verified on the package reinstalled to `/Applications`. That installed pass invoked no Apply, Capture, TCC, login, or system-setting mutation and does not prove full keyboard traversal, VoiceOver, focused-control AX state, ColorSync, audio, or IPv4 behavior.
...
| Empty-state horizontal layout | An attached AppKit boundary converts native `T3/L11/B5/R2` to hosted `T3/L0/B5/R0`, preserving content at `x16…352/y21…297`. A zero/filtered/unfiltered three-way raster proves the filtered asymmetric result matches baseline and the unfiltered control moves | All 20 installed measurements reported exact center delta `0`; empty-tray Capture CTA prominence remains a P2 follow-up |
...
| Action disposition and handoff | Every action is mapped; stay-open cannot close; visible/key order, bounded non-key liveness, independently cancellable coalesced callers, red-close, same-turn immediate reopen, stale completion, failure, and quit-only termination pass | Installed Settings/workflow geometry and Escape close passed; full native focus-transfer timing, keyboard traversal, and focused-control AX observation remain pending |
| App-lifetime state/workflows | Dismiss/reopen task survival, deletion focus, persistent-window ownership, synthetic permission suppression, minimum-size adaptive actions, safe keyboard focus policy, and accessibility focus intent pass | Installed minimum clamps passed. Full keyboard and VoiceOver speech/rotor walkthrough remain pending |
| Catalog parity and bilingual-path rendering | The [current installed audit](UI-UX-SIMPLIFICATION-INSTALLED-AUDIT-2026-07-17.md) and [installed evidence index](evidence/ui-ux-installed-2026-07-17/README.md) record the final installed/synthetic set. The [preceding structural audit](UI-STRUCTURAL-RELIABILITY-AUDIT-2026-07-17.md) retains the 680×480 Korean Settings and two 520×360 Korean workflow accessibility-text fixtures. Some system-localized controls remain English in the forced-locale host | Native linguistic review, real text size/contrast/transparency, full VoiceOver, and complete focused-control AX observation remain pending |

## Platform

| Area | Target | Current evidence |
| --- | --- | --- |
| macOS | 14 Sonoma or later | Deployment target is 14.0. The final integrated gate passed 446 checks on 2026-07-17: 144 XCTest cases, 301 Swift Testing cases across 38 suites, and one separately executed native `NSPopover` regression. Earlier gates remain historical evidence |
...
| Distribution | Direct no-Developer-ID DMG | Current `artifacts/Desk-Setup-Switcher-0.1.0-unsigned.dmg`, SHA-256 `82b05e6a3b1b20978c85c4d82d0976237a731c76c07bcd7d1729a84e3e0b6f21`, passed checksum, mount, metadata/resources, `x86_64 arm64`, and ad-hoc/no-Developer-ID signature verification. The `/Applications` reinstall passed with executable SHA-256 `40876bd671dd4286fa4684192097f6dbd702df899bccf8efb588b244c3d27305`. Nothing is published and the quarantined Gatekeeper path remains pending |
...
| --- | --- | --- |
| App | Background/menu-bar-only lifecycle | `LSUIElement` and the app-owned `NSStatusItem`/`NSPopover` source are build/package verified. Persistent Settings/workflow presentation has a liveness deadline, consumer-local cancellation, red-close detachment, immediate-reopen defense, and identity-guarded stale completion. The current package was reinstalled and its bounded tray/window geometry passed; full focus and accessibility behavior remain pending |
...
| App | Accessibility | Current source contains icon labels/help, non-color state and validation text, deterministic deletion focus, adaptive workflow actions, safe Cancel/Revert/Close keyboard targets, heading/status accessibility focus intent, shortcuts, and one combined remaining-time value. Attached/offscreen hosts improve native row and raster coverage but do not prove VoiceOver speech/rotor/focus, keyboard-only first-responder rings, contrast, transparency, or real macOS text-size behavior |
| App | System, About, and diagnostics presentation | Profiles/System/About is the primary navigation. System keeps login and permission guidance visible and describes diagnostics as optional troubleshooting; an on-demand advanced sheet retains browsing, refresh, and confirmed clearing. About keeps repository, issue, MIT license, and privacy links. Current English/Korean System and diagnostics screenshots plus read-only AX logs passed static review; Advanced Diagnostics' 700-point minimum versus Settings' 680-point minimum and minimum/AX fixtures for About remain P2 follow-ups |
...
| Profiles | Current-settings capture | Tray Capture creates and selects a new reviewable profile. The removed draft-refresh action cannot silently replace an open draft. Empty-tray Capture prominence remains a bounded P2 follow-up; the installed audit did not invoke Capture |
| Profiles | Flat typed setting-value editor | Display/Audio/Network are always expanded; each leaf owns Include and there is no group/option disclosure state. `VisibleSettingRegistry` projects only nine complete setting kinds from typed runtime catalogs; unsupported, read-only, missing, or ambiguous controls are absent as new choices. A persisted included Audio, ColorSync, or service-IPv4 value that becomes unavailable keeps a warning plus Include-off repair control. Standard/minimum/large-text/dark/Korean evidence includes the combined 680×480 Korean accessibility-text fixture; Included-toggle meaning, `DisclosureGroup` VoiceOver behavior, and focused-control AX observation remain P2/manual work |
| Profiles | Value-first summaries, icon preservation, and technical disclosures | Unit verified; current v2 English/Korean detached-host evidence covers long names, readiness text, symbols, overflow, and destructive state. Installed popover centering passes; profile-row action click-through remains pending |
...
| Profiles | Import/export | Existing size/schema/semantic, duplicate-ID, source-protection, and no-overwrite behavior is mock verified. The full gate passed deterministic import/codec coverage proving unsupported applicability is normalized on decode/import while values are preserved. Duplicate/Reorder/Import/Export hierarchy and dirty-draft Export semantics remain bounded P2 work |
...
| Stable Settings workspace | Policy and owned-window geometry tests cover one 210-point sidebar + 390-point-minimum editor anatomy, resizable style, 680×480 minimum, fixed footer/action bars, at-most-one refresh adoption per presentation, no reset when invoking an already-visible window, and stable window ownership | Attached/offscreen evidence passes. Current installed `500×300`→`680×480` clamping and ten `900×568` opens pass; full content-state, flicker, keyboard, and focused-control AX review remains manual |
...
## Tray/settings end-to-end and structural evidence recorded through 2026-07-17

The [current UI/UX simplification and installed-app audit](UI-UX-SIMPLIFICATION-INSTALLED-AUDIT-2026-07-17.md) records the final root cause and evidence. Its integrated gate passes 446 checks: 144 XCTest cases, 301 Swift Testing cases across 38 suites, and one separately executed native `NSPopover` regression. Swift/Xcode Debug and Release, Analyze, checksum, mounted metadata/resources/`x86_64 arm64`, and ad-hoc signature verification pass. Current local DMG SHA-256 is `82b05e6a3b1b20978c85c4d82d0976237a731c76c07bcd7d1729a84e3e0b6f21`; the separately authorized `/Applications` reinstall verifies executable SHA-256 `40876bd671dd4286fa4684192097f6dbd702df899bccf8efb588b244c3d27305`. The structural, settings-lifecycle, and UI-stability audits remain preceding evidence.
...
### Bounded P2 UI follow-up

The next UI pass is limited to empty-tray Capture prominence; Duplicate/Reorder/Import/Export hierarchy; dirty-draft Export semantics; Included-toggle ambiguity; VoiceOver behavior for `DisclosureGroup`; the Advanced Diagnostics 700-point versus Settings 680-point minimum; minimum-size and AX fixtures for About, protected-change safety, and results; `⌘,` tab forcing; and generic storage-error prominence. It must not be reported as a completed VoiceOver or focused-control AX walkthrough.
