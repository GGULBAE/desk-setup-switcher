...
enum ProfileEditorNetworkSelectionPolicy {
  static func updatingInclusion(
    ...
    isIncluded: Bool,
    runtimeChoices: [NetworkServiceIPv4Settings]
  ) -> [NetworkServiceIPv4Settings] {
    var updated = targets
    if let index = updated.firstIndex(where: { $0.identity == selectedIdentity }) {
      updated[index].configuration.isIncluded = isIncluded
      if isIncluded, updated[index].configuration.value == nil {
        updated[index].configuration.value = .dhcp
      }
    } else if isIncluded,
      var runtime = runtimeChoices.first(where: { $0.identity == selectedIdentity })
    {
      runtime.configuration.isIncluded = true
      runtime.configuration.value = runtime.configuration.value ?? .dhcp
...
enum ProfileEditorAudioVolumeCapabilityResolver {
  static func resolve(
    ...
    guard
      ...
      let entry = catalog.first(where: {
        $0.role == role && $0.deviceUID == targetDeviceUID
      })
    else {
      return ProfileEditorAudioVolumeCapability(isWritable: false, suggestedValue: nil)
    }
    return ProfileEditorAudioVolumeCapability(
...
enum ProfileEditorUnavailableIncludedSettingPolicy {
  ...
  static func showsRepairControl(
    ...
    isIncluded && hasRuntimeEvidence && !isRuntimeAvailable
  }

  static func unavailableNetworkServiceIndices(
    in targets: [NetworkServiceIPv4Settings],
    kind: NetworkServiceKind,
    availableIdentities: Set<NetworkServiceIdentity>
  ) -> [Int] {
    targets.indices.filter { index in
      let target = targets[index]
      return target.identity.kind == kind
...
struct ProfilesSettingsView: View {
  ...
  private var editorActionBar: some View {
    HStack(spacing: 10) {
      if let activityStatus = editorActivityStatus {
        Label(activityStatus.title, systemImage: activityStatus.symbol)
          .font(.caption)
          .foregroundStyle(editorActivityIsError ? .red : .secondary)
          .lineLimit(1)
          .truncationMode(.tail)
          .help(activityStatus.title)
          .accessibilityLabel(
            appLocalized("Profile editor status: \(activityStatus.title)")
          )
      }
      Spacer(minLength: 8)
      revertDraftButton
  ...
  private var editorActivityStatus: (title: String, symbol: String)? {
    switch profileEditor.activity {
    case .saved:
      nil
    case .changed:
      (appLocalized("Unsaved changes"), "pencil.circle")
    case .saving:
      (appLocalized("Saving profile…"), "hourglass")
    case .capturing:
      (appLocalized("Reading current settings…"), "hourglass")
...
private struct ProfileEditorForm: View {
  ...
  private func stepTitle(_ group: SettingGroup) -> String {
    switch group {
  ...
  @ViewBuilder
  private var networkSimpleOptions: some View {
  ...
  @ViewBuilder
  private func simpleControlRow<Content: View>(
  ...
  private var simpleNetworkSelectedIdentity: NetworkServiceIdentity? {
    let availableIdentities = Set(simpleNetworkServiceChoices.map(\.identity))
    if let selectedNetworkIdentity,
      availableIdentities.contains(selectedNetworkIdentity)
    {
      return selectedNetworkIdentity
    }
    return profile.settings.network.value.serviceIPv4.first {
      $0.configuration.isIncluded && availableIdentities.contains($0.identity)
    }?.identity ?? simpleNetworkServiceChoices.first?.identity
  }

  private var simpleNetworkSelectionBinding: Binding<NetworkServiceIdentity?> {
    Binding(
      get: { simpleNetworkSelectedIdentity },
      set: { selectedIdentity in
        selectedNetworkIdentity = selectedIdentity
      }
    )
  }

  private var networkIncludedBinding: Binding<Bool> {
    Binding(
      get: {
        guard let selectedIdentity = simpleNetworkSelectedIdentity else { return false }
        return profile.settings.network.value.serviceIPv4.first {
          $0.identity == selectedIdentity
        }?.configuration.isIncluded ?? false
      },
      set: { isIncluded in
        guard let selectedIdentity = simpleNetworkSelectedIdentity else { return }
        profile.settings.network.value.serviceIPv4 =
          ProfileEditorNetworkSelectionPolicy.updatingInclusion(
            in: profile.settings.network.value.serviceIPv4,
            selectedIdentity: selectedIdentity,
            isIncluded: isIncluded,
            runtimeChoices: simpleNetworkServiceChoices
          )
      }
    )
  }
  ...
  private var orderedVisibleGroups: [SettingGroup] {
    ProfileEditorStepPolicy.orderedGroups
  }
  ...
  private func audioVolumeOption(
    ...
    let capability = audioVolumeCapability(role: role)
  ...
  private func audioVolumeCapability(
    role: AudioVolumeCatalogRole
  ) -> ProfileEditorAudioVolumeCapability {
    let selectedDevice: SettingOption<String?>
    let currentDeviceUID: String?
    switch role {
    case .input:
      selectedDevice = profile.settings.audio.value.defaultInputUID
      currentDeviceUID = systemSnapshot?.profileSettings.audio.value.defaultInputUID.value
    case .output:
    ...
    return ProfileEditorAudioVolumeCapabilityResolver.resolve(
      role: role,
      selectedDevice: selectedDevice,
      currentDeviceUID: currentDeviceUID,
      catalog: systemSnapshot?.audioVolumeControlCatalog ?? []
    )
  }
  ...
  private var hasIncludedUnavailableNetworkFields: Bool {
    NetworkServiceKind.allCases.contains { kind in
      !unavailableIncludedNetworkServiceIndices(kind: kind).isEmpty
    }
  }
  ...
  private func unavailableIncludedNetworkServiceIndices(
    kind: NetworkServiceKind
  ) -> [Int] {
    guard systemSnapshot != nil else { return [] }
    let availableIdentities = Set(availableNetworkServices(kind: kind).map(\.identity))
    return ProfileEditorUnavailableIncludedSettingPolicy.unavailableNetworkServiceIndices(
      in: profile.settings.network.value.serviceIPv4,
      kind: kind,
...
extension View {
  fileprivate func simpleSettingsSurface() -> some View {
    background(
