...
/// Disclosure is presentation-only state. Keeping it in a separate value makes
/// it impossible for collapse/expand actions to mutate profile inclusion or
/// any saved target value.
public struct DisclosureState<Item>: Equatable, Sendable
where Item: Hashable & Sendable {
  public private(set) var expandedItems: Set<Item>

  public init(expandedItems: Set<Item> = []) {
  ...
  public mutating func expand(_ item: Item) {
    expandedItems.insert(item)
  }

  public mutating func setExpanded(_ isExpanded: Bool, for item: Item) {
    if isExpanded {
      expandedItems.insert(item)
    } else {
  ...
  public mutating func toggle(_ item: Item) {
    if expandedItems.remove(item) == nil {
      expandedItems.insert(item)
    }
  }
