...
- The page presents the current nine-setting Display/Sound/Keyboard scope: main display, resolution, output device/volume/mute, input device/volume, key repeat speed, and repeat delay. Keyboard brightness, pointer, scrolling, function-key, Network, and other legacy values remain dormant compatibility data, not current site capabilities; Capture does not request Input Monitoring access.
