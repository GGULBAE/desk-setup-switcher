...
- The current profile surface is **Display + Sound** only: main display, resolution, output device/volume/mute, and input device/volume. Capture queries only Display/Audio and does not request Location permission. Network and other historical fields remain dormant compatibility data and never reach the current Apply path. Interpret each current capability using the verification levels in the [support matrix](docs/SUPPORT-MATRIX.md); mock-verified or live-read behavior is not hardware-mutation verification.
